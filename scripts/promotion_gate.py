...
def requires_source_content_match(path: str) -> bool:
    """Sensitive candidate code must be byte-identical to tested research code.

    The live champion manifest is intentionally excluded: research branches are forbidden
    from mutating that production selector, so the integration branch may make the explicit
    selector change after the underlying candidate implementation has been evidenced.
    """
...
def research_verdict(source: dict[str, Any]) -> str | None:
    ...
    for comment in source.get("comments") or []:
        if isinstance(comment, dict):
            events.append(
                (
                    str(comment.get("createdAt") or comment.get("created_at") or ""),
    ...
    for review in source.get("reviews") or []:
        if isinstance(review, dict):
            events.append(
                (
                    str(review.get("submittedAt") or review.get("submitted_at") or ""),
...
def validate_windows(
    windows: list[dict[str, Any]], minimum: int, require_non_overlap: bool
) -> list[str]:
    errors: list[str] = []
...
def validate_evidence(
    ...
    errors.extend(
        validate_windows(
            windows,
            min_passes,
            bool(policy.get("require_non_overlapping_test_windows", True)),
        )
    )
...
def evaluate(
    ...
    policy: dict[str, Any],
    now: int,
    require_approval_label: bool = False,
) -> dict[str, Any]:
    errors: list[str] = []
    ...
    errors.extend(
        check_errors(
            candidate.get("statusCheckRollup") or [],
            REQUIRED_CANDIDATE_CHECKS,
            "candidate ",
        )
    )
    ...
    if kind == "economic":
        if evidence is None:
            ...
            errors.extend(
                validate_evidence(evidence, candidate, source, alpha_config, policy, now)
            )
...
def main() -> int:
    ...
    result = evaluate(
        ...
        require_approval_label=args.require_approval_label,
    )
    ...
    if result["errors"]:
        ...
        lines.extend(
            [
                "",
                "All applicable objective gates passed. The controller may authorize paper promotion.",
            ]
        )
