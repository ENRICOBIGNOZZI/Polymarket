...
def validate_context(
    ...
    *,
    ...
    universal_state = architecture.get("universal_state")
    if not isinstance(universal_state, list) or set(universal_state) != EXPECTED_STATE:
        errors.append("quantitative_architecture.universal_state must contain the four canonical state blocks")
    alpha_engines = architecture.get("alpha_engines")
    if not isinstance(alpha_engines, list) or set(alpha_engines) != EXPECTED_ENGINES:
    ...
    if assignments.get("paper-server-deploy") != "remote":
        errors.append("paper-server-deploy must use the remote profile")
    if assignments.get("paper-server-health") != "remote":
        errors.append("paper-server-health must use the remote profile")
    ...
    if scheduler_id is not None:
        if scheduler_id not in assignments:
...
def render_markdown(data: dict[str, Any], scheduler_id: str | None, errors: list[str]) -> str:
    ...
    lines = [
        ...
        f"- research mode: `{architecture.get('operating_mode', {}).get('research_engine', '?') if isinstance(architecture.get('operating_mode'), dict) else '?'}`",
        f"- maximum drawdown operating ratio: `{architecture.get('portfolio_and_risk', {}).get('maximum_drawdown_ratio', '?') if isinstance(architecture.get('portfolio_and_risk'), dict) else '?'}`",
        "",
...
def main() -> int:
    ...
    if args.output_json:
        snapshot = {
            ...
            "profile": (
                data.get("scheduler_contract", {}).get("assignments", {}).get(args.scheduler_id, "registry-wide")
                if data
                else "invalid"
            ),
