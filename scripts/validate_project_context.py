...
def validate(root: Path) -> tuple[list[str], list[str]]:
    ...
    for path in (context_path, registry_path, champion_path, ssh_path, snapshot):
        if not path.is_file(): errors.append(f"missing context surface: {path.relative_to(root)}")
    if errors: return errors, notes

    context = load(context_path); registry = load(registry_path); champion = load(champion_path)
    ...
    if context.get("schema_version") != 1: errors.append("project context schema_version must equal 1")
    if context.get("repository") != "ENRICOBIGNOZZI/Polymarket": errors.append("repository mismatch")
    if context.get("canonical_branch") != "main": errors.append("canonical branch must be main")
    if context.get("deployment_ref") != "paper-validated": errors.append("deployment ref must be paper-validated")
    policy = context.get("context_policy") if isinstance(context.get("context_policy"), dict) else {}
    if policy.get("scope") != "entire_repository": errors.append("context scope must be entire_repository")
    if policy.get("require_full_worktree_inventory") is not True: errors.append("full worktree inventory must be required")
    if policy.get("require_operator_directives") is not True: errors.append("operator directives must be required")

    if champion.get("enabled") is not False:
        errors.append("cleanup transition requires live champion to be explicitly disabled")
    for key in ("version", "loop", "config", "run_root"):
        if champion.get(key) is not None: errors.append(f"disabled champion requires {key}=null")
    ...
    if directives.get("schema_version") != 1: errors.append("operator directives schema_version must equal 1")
    if directives.get("authority") != "latest_explicit_user_instruction": errors.append("operator authority mismatch")
    if directives.get("repository") != "ENRICOBIGNOZZI/Polymarket": errors.append("operator repository mismatch")
    ...
    if architecture.get("operational_champion_may_be_absent") is not True:
        errors.append("operator directive must allow no champion during cleanup")
    if architecture.get("cleanup_sequence") != "delete_legacy_immediately_then_promote_only_validated_v7":
        errors.append("operator cleanup sequence is stale")
    ...
    for key in ("max_trade_fraction", "max_market_fraction", "max_event_fraction", "max_gross_fraction", "hard_arb_max_trade_fraction"):
        if num(auth.get(key)) != 1.0: errors.append(f"operator V7 authorization requires {key}=1.0")
    if num(auth.get("fractional_kelly_ceiling")) != 0.25: errors.append("fractional Kelly ceiling must be 0.25")
    if num(auth.get("max_drawdown")) != 0.15: errors.append("max drawdown must be 0.15")
    ...
    if missing: errors.append("operator directives missing active scheduler assignments: " + ", ".join(missing))
    ...
    forbidden_ids = {"v6-live-data-research", "v6-market-cache-relay", "live-paper-validation", "paper-server-deploy", "paper-server-health", "forward-maker-research"}
    present = sorted(active_ids.intersection(forbidden_ids))
    if present: errors.append("retired schedulers still active: " + ", ".join(present))

    priorities = directives.get("current_priority_order")
    if not isinstance(priorities, list) or len(priorities) < 8: errors.append("complete current priority order is required")
    forbidden = directives.get("forbidden_regressions")
    if not isinstance(forbidden, list) or not any("$125" in str(x) for x in forbidden): errors.append("$125 rollback must remain forbidden")

    for rel in [str(x) for x in context.get("required_surfaces", [])]:
        if not (root / rel).exists(): errors.append(f"required repository surface is missing: {rel}")
    ...
    if grafana.get("active") is not False or grafana.get("dashboard_uid") is not None or grafana.get("dashboard_file") is not None:
        errors.append("champion-specific Grafana must be inactive during no-champion transition")

    ssh = ssh_path.read_text(encoding="utf-8")
    for required in ("Host polymarket", "HostName 100.104.183.109", "Port 22", "User enrico"):
        if required not in ssh: errors.append(f"canonical SSH config missing: {required}")
    ...
    for item in schedulers:
        ...
        rel = str(item.get("workflow") or ""); path = root / rel
...
def render(errors: list[str], notes: list[str]) -> str:
    lines = ["# Scheduler project-context validation", "", f"- active schedulers: {len(notes)}", f"- errors: {len(errors)}", "- operational champion: disabled"]
    if errors: lines += ["", "## Errors", *[f"- {e}" for e in errors]]
    else: lines += ["", "V3-V6 are retired; active schedulers have full-worktree context and no champion is required."]
    return "\n".join(lines) + "\n"


def main() -> int:
    p = argparse.ArgumentParser(); p.add_argument("--root", default="."); p.add_argument("--output", default="project-context-validation.md")
    a = p.parse_args(); root = Path(a.root).resolve()
    try: errors, notes = validate(root)
    except Exception as exc: errors, notes = [str(exc)], []
    report = render(errors, notes); Path(a.output).write_text(report, encoding="utf-8"); print(report, end="")
