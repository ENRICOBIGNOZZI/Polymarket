...
def validate(root: Path) -> tuple[list[str], list[str]]:
    ...
    # The current explicit operator directive gives the event bridge only two
    # dispatch authorities: Promotion Controller after validator completion and
    # Integration Merge after a successful controller completion. Keep this
    # machine-checkable so an operator/* PR cannot silently enlarge that role.
    bridge = next(
        (
            raw
