...
def full_depth_sell_vwap(levels: list[tuple[float, float]], shares: float) -> float | None:
    """Executable VWAP for selling `shares` into the full visible bid book.

    Returns None when displayed depth cannot fully liquidate the requested size;
    partial-depth proceeds are never promoted to a full-exit price.
    """
...
def _exit_requirements(run_dir: Path) -> dict[str, float]:
    """Upper-bound shares that may need executable liquidation this invocation."""
    ...
    for token in set(position_shares) | set(order_shares) | set(watch_shares):
        ...
        # A partially filled resting order can add to an existing position before
        # the same tick reaches the hold-time/kill liquidation path. If the kill
        # switch was already active, any residual order can likewise fill from a
        # delayed pre-expiry event and require immediate liquidation.
...
# Preserve canonical adapter helpers after compatibility re-exports.
