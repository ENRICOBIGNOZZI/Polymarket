...
def nested_short_horizon_ablation(
    ...
    pvalue = ext.block_bootstrap_pvalue(
        cluster_values,
        block=max(1, ext.integer(backtest.get("bootstrap_block"), 5)),
        reps=max(100, ext.integer(backtest.get("bootstrap_reps"), 1000)),
        seed=20260824 + sum(ord(char) for char in source + feature_name) + int(horizon_seconds),
    )
