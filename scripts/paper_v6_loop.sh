...
rm -f "$RUN_ROOT"/intents.csv "$RUN_ROOT"/local_factor_raw.csv "$RUN_ROOT"/local_factor_intents.csv \
      "$RUN_ROOT"/relation_intents_raw.csv "$RUN_ROOT"/relation_guarded.csv "$RUN_ROOT"/relation_queue.csv \
...
cleanup(){ for p in "$rec_pid" "$broker_pid" "$external_pid" "$proxy_pid";do ((p>0))&&kill "$p" 2>/dev/null||true;done;for p in "$rec_pid" "$broker_pid" "$external_pid" "$proxy_pid";do ((p>0))&&wait "$p" 2>/dev/null||true;done; }
trap cleanup EXIT INT TERM
start_proxy
proxy_ready=0
for _ in {1..50};do if curl -fsS "http://127.0.0.1:${MARKET_PROXY_PORT}/healthz" >/dev/null 2>&1;then proxy_ready=1;break;fi;kill -0 "$proxy_pid" 2>/dev/null||break;sleep 0.1;done
...
while true;do
  ...
  # Discovery-only until the per-leg receive-time/legging contracts from Fast Arb are ported.
  ...
  if ((now-last_factor>=30));then
    rm -f "$RUN_ROOT/local_factor_raw.csv" "$RUN_ROOT/local_factor_intents.csv" "$RUN_ROOT/stat_arb_pairs_diagnostic.csv"
    ...
    python3 scripts/v6_local_factor_v3.py --config "$CONFIG" --output "$RUN_ROOT/local_factor_raw.csv" --status "$RUN_ROOT/local_factor_status.json" --trade-tape "$RUN_ROOT/trade_tape.csv" --markets 700 --min-liquidity "$MIN_LIQUIDITY" --lookback-hours 336 --fidelity-minutes 60 --max-clusters 30 --min-common-points 36 --min-z 0.75 --fdr 0.15 --min-edge "$INTENT_MIN_EDGE" --max-trade-usd 100 --slippage-bps 5 --flow-lookback-seconds 900 --min-fill-probability 0.0 --exit-buffer-seconds 900 >"$RUN_ROOT/local_factor_latest.log" 2>"$RUN_ROOT/local_factor_errors.log"||true
    python3 scripts/v6_bundle_risk_guard.py --config "$CONFIG" --input "$RUN_ROOT/local_factor_raw.csv" --output "$RUN_ROOT/local_factor_intents.csv" --status "$RUN_ROOT/local_factor_execution_status.json" --trade-tape "$RUN_ROOT/trade_tape.csv" --min-edge "$INTENT_MIN_EDGE" --min-joint-fill 0.0000001 --flow-lookback-seconds 900 --fill-horizon-seconds 180 --slippage-bps 5 --ttr-buffer-seconds 900 >>"$RUN_ROOT/local_factor_execution.log" 2>&1||true
    rebuild_intents; last_factor=$now
  fi

  if ((now-last_relation>=15));then
    rm -f "$RUN_ROOT/relation_intents.csv" "$RUN_ROOT/relation_queue.csv"
    ...
    python3 scripts/v6_queue_filter.py --config "$CONFIG" --input "$RUN_ROOT/relation_guarded.csv" --output "$RUN_ROOT/relation_queue.csv" --status "$RUN_ROOT/queue_filter_status.json" --trade-tape "$RUN_ROOT/trade_tape.csv" --min-edge "$INTENT_MIN_EDGE" --reserve-bps 2 --flow-lookback-seconds 900 --horizon-seconds 180 --min-leg-fill-probability 0.002 --min-joint-fill-probability 0.0000001 --target-leg-fill-probability 0.10 --max-improve-ticks-per-leg 12 >>"$RUN_ROOT/queue_filter.log" 2>&1||true
    python3 scripts/v6_bundle_risk_guard.py --config "$CONFIG" --input "$RUN_ROOT/relation_queue.csv" --output "$RUN_ROOT/relation_intents.csv" --status "$RUN_ROOT/relation_execution_status.json" --trade-tape "$RUN_ROOT/trade_tape.csv" --min-edge "$INTENT_MIN_EDGE" --min-joint-fill 0.0000001 --flow-lookback-seconds 900 --fill-horizon-seconds 180 --slippage-bps 5 --ttr-buffer-seconds 900 >>"$RUN_ROOT/relation_execution.log" 2>&1||true
