...
# V6 market discovery uses the validated resilient read-only proxy. Latest main
# provides keyset pagination, bounded CLOB fallback/cache and deadline handling.
...
# Capital-isolated sleeves. The aggregate paper envelope is controlled by the
# parent allocation and shared configured fractions; all execution stays paper-only.
...
# Close the startup race: materialize the fail-closed external feed before the engine.
...
while true;do
  ...
  # Hard complete-set arbitrage: verified fee semantics and multi-level VWAP.
