...
def materialize(input_path: Path, output_path: Path, now: int,
                min_confidence: float, max_age_seconds: int) -> int:
        ...
        for raw in input_path.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            if current is None or rank > current["rank"]:
                best[market] = {
                    ...
                    "source": f"external_intelligence:{row.get('source', 'unknown')}:{row.get('source_id', '')}",
                    "timestamp": timestamp,
                }
...
def main() -> int:
    ...
    count = materialize(
        args.input, args.output, now,
        max(0.0, args.min_confidence), max(1, args.max_age_seconds),
    )
