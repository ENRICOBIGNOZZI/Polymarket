#!/usr/bin/env python3
"""Quantify the execution hurdle for maker-positive B2 multi-leg candidates.

This is a read-only research diagnostic. It consumes a public live-smoke JSON
snapshot and does not submit orders or alter production configuration.
"""
...
def break_even_completion_probability(maker_edge: float, taker_edge: float) -> float | None:
    """Probability of maker completion needed to offset taker fallback economics."""
        ...
        rows.append(
            {
                ...
                "break_even_full_bundle_completion_probability": break_even,
                "iid_per_leg_fill_floor_for_break_even": per_leg,
                "expected_edge_if_completion_80pct": expected_edge_at_completion(0.80, maker, taker),
                "expected_edge_if_completion_90pct": expected_edge_at_completion(0.90, maker, taker),
                "expected_edge_if_completion_95pct": expected_edge_at_completion(0.95, maker, taker),
                "expected_edge_if_completion_99pct": expected_edge_at_completion(0.99, maker, taker),
                "interpretation": (
                    "IID per-leg fill floor is a diagnostic only; real leg fills are dependent and must be measured jointly."
                ),
            }
        )
    rows.sort(key=lambda row: float(row["maker_entry_net_edge"]), reverse=True)
    return {
        "schema": "polymarket_hf_b2_completion_hurdle_v1",
