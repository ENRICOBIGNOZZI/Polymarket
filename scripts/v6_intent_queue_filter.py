...
def request_json(url: str, payload: Any | None = None, timeout: int = 20) -> Any:
    ...
    req = urllib.request.Request(
        url,
        data=data,
        headers={"User-Agent": "polymarket-v6-paper/intent-queue-filter", "Content-Type": "application/json"},
    )
...
def fetch_books(clob: str, tokens: list[str]) -> tuple[dict[str, dict[str, Any]], list[str]]:
    ...
    for start in range(0, len(unique), 80):
        ...
        try:
            rows = request_json(f"{clob.rstrip('/')}/books", [{"token_id": x} for x in batch])
        except Exception as exc:
...
def queue_at_limit(book: dict[str, Any], limit: float) -> float:
    return sum(q for p, q in book["bids"] if abs(p - limit) <= 1e-9)
...
def evaluate_bundle(
    ...
    if any(row.get("mode", "").upper() != "MAKER" for row in rows):
        return False, "non_maker_bundle", math.inf, []
    expected_edge = min(finite(r["expected_edge"], -math.inf) for r in rows)
    max_notional = min(finite(r["max_notional"], 0.0) for r in rows)
    capital_per_unit = sum(
        max(0.0, finite(r["weight"], 0.0)) * max(0.0, finite(r["limit_price"], 0.0))
        for r in rows
    )
    ...
    for row in rows:
        ...
        # The multileg broker is passive. A stale or crossing limit is rejected
        # here instead of reserving capital and relying on later cancel/replace.
...
def main() -> int:
    ...
    token_map, failures = fetch_tokens(str(cfg["gamma_url"]), [row for rows in grouped.values() for row in rows])
    books, book_failures = fetch_books(str(cfg["clob_url"]), list(token_map.values()))
