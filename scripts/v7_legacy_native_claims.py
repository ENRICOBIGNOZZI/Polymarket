...
def _native_carryover_claims(current: Path, target_sha: str) -> dict[tuple[str, str], int]:
    value = {}
    ...
    if path.is_file():
        try:
            ...
            raise ValueError("native_carryover_registry_invalid") from exc
    if not value:
        return {}
    if (
        not isinstance(value, dict)
        or value.get("schema") != "polymarket_v7_native_carryover_exposure_v1"
        or value.get("paper_only") is not True
        or value.get("authenticated_execution") is not False
        or value.get("real_order_submission") is not False
        or value.get("target_model_sha") != target_sha
        or not isinstance(value.get("markets"), list)
    ):
        raise ValueError("native_carryover_registry_invalid")
    ...
    for row in value["markets"]:
        ...
        amount = row.get("claim_microdollars")
        if (
            ...
            or amount <= 0
            or (sha, market) in claims
        ):
            raise ValueError("native_carryover_registry_invalid")
        claims[(sha, market)] = amount
    total = value.get("total_unsettled_microdollars")
    if not isinstance(total, int) or isinstance(total, bool) or total != sum(claims.values()):
        raise ValueError("native_carryover_registry_invalid")
    return claims
...
def build_registry(*, current_run_root: Path, target_sha: str, scan_parent: Path) -> dict[str, Any]:
    ...
    covered = _native_carryover_claims(current, target_sha)
            ...
            for market, claim in sorted(exposure["unsettled_market_claims_microdollars"].items()):
                ...
                if key in covered:
                    if covered[key] != claim:
                        raise ValueError("native_carryover_legacy_claim_mismatch")
                    continue
                if key in seen_legacy:
                    raise ValueError("legacy_claim_duplicate_across_roots")
                seen_legacy[key] = claim
    ...
    body = {
        ...
        "sources": sources,
        "native_carryover_excluded_markets": len(covered),
        "native_carryover_excluded_microdollars": sum(covered.values()),
