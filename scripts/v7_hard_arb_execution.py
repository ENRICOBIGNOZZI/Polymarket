...
from polymarket_fees import FeeDetails, parse_fee_details, resolve_fee_details

import v7_hard_arb_base as hard_base
...
def round_fee_usdc(value: float) -> float:
    ...
    return float(rounded) if rounded >= Q else 0.0
...
def fee_amount(shares: float, price: float, details: FeeDetails, taker: bool = True) -> float:
    if (
        ...
        return 0.0
    return round_fee_usdc(
        shares
        * details.rate
        * (price * (1 - price)) ** max(0.0, details.exponent)
    )
...
def _levels(levels: Iterable[tuple[float, float]], buy: bool) -> list[tuple[float, float]]:
    ...
    for price, quantity in levels:
        price = finite(price)
        quantity = finite(quantity, 0.0)
        if math.isfinite(price) and 0 < price < 1 and quantity > 0:
            out.append((price, quantity))
    out.sort(key=lambda row: row[0], reverse=not buy)
    return out
...
def walk_book_for_shares(
    ...
    *,
    ...
    for price, quantity in _levels(levels, buy):
        take = min(remaining, quantity)
        if take <= 0:
            continue
        stressed_price = (
            min(0.999999, price * (1 + stress))
            if buy
            else max(0.000001, price * (1 - stress))
        )
        raw_cash += take * price
        stressed_cash += take * stressed_price
        fees += fee_amount(take, stressed_price, details, True)
        filled += take
        remaining -= take
        if remaining <= 1e-10:
            break
    ...
    all_in = (
        (stressed_cash + fees) / filled
        if buy
        else (stressed_cash - fees) / filled
    )
    return BookFill(
        target,
        filled,
    ...
    for key, out in (("bids", bids), ("asks", asks)):
        for row in raw.get(key, []):
            if isinstance(row, dict):
                ...
                quantity = finite(row.get("size"), 0.0)
                if math.isfinite(price) and 0 < price < 1 and quantity > 0:
                    out.append((price, quantity))
...
def _hard_fee(
    ...
    del cfg
    details = resolve_fee_details(raw, clob, hard_base.get_json)
...
def _size(
    ...
    if (
        plan is None
        or plan[0] > room + 1e-9
        or 1 - plan[0] / min_order <= edge_gate
    ):
        return None
    lo, hi = min_order, max_shares
    best = (lo, *plan)
    for _ in range(34):
        shares = (lo + hi) / 2
        plan = _plan(live, tokens, fees, shares, slip)
        if plan and plan[0] <= room + 1e-9 and 1 - plan[0] / shares > edge_gate:
            best = (shares, *plan)
            lo = shares
        else:
            hi = shares
...
def _unwind(
    ...
    for leg in reversed(legs):
        ...
        allocated = cost_basis * min(1.0, sold / max(shares, 1e-12))
        ...
        if remaining > 1e-9:
            ...
            row["cost"] = max(0.0, cost_basis - allocated)
            residual.append(row)
    residual.reverse()
...
def _hard_main() -> int:
    ...
    for event_id, bundle in list(aborting.items()):
        ...
        residual, received, pnl = _unwind(clob, legs, fee_map, args.slippage_bps)
        cash += received
        realized += pnl
        if residual:
            bundle["legs"] = residual
            bundle["cost"] = sum(finite(row.get("cost"), 0.0) for row in residual)
        else:
            del aborting[event_id]

    locked = sum(float(bundle["cost"]) for bundle in open_bundles.values())
    abort_cost = sum(float(bundle.get("cost") or 0) for bundle in aborting.values())
    ...
    if not killed and not aborting:
        try:
            event_ids = hard_base.discover_event_ids(
                gamma, args.markets, args.min_liquidity, args.max_events
            )
        except Exception as exc:
            event_ids = []
        ...
        for event_id in event_ids:
            if event_id in open_bundles:
                continue
            try:
                markets = hard_base.event_spec(gamma, event_id)
                if markets is None:
                    continue
                tokens: list[str] = []
                fee_map: dict[str, FeeDetails] = {}
                ...
                for market in markets:
                    yes, _ = hard_base.market_tokens(market) or ("", "")
                    if not yes:
                        raise RuntimeError("missing_yes_token")
                    tokens.append(yes)
                    market_by_token[yes] = market
                    fee_map[yes] = _hard_fee(market, clob, cfg, sources)
                ...
                current_equity = max(1.0, equity)
                room = min(
                    args.max_trade_usd,
                    max(0.0, max_gross * current_equity - locked - abort_cost),
                    max(0.0, max_event * current_equity),
                    cash,
                )
                plan = (
                ...
                if plan:
                    cost_per_share = plan[0] / min_order
                    edge = 1 - cost_per_share
                ...
                for index, token in enumerate(order):
                    if index and args.leg_latency_ms > 0:
                        time.sleep(args.leg_latency_ms / 1000.0)
                    remaining_tokens = order[index:]
                    fresh = books(clob, remaining_tokens)
                    if any(item not in fresh for item in remaining_tokens):
                    ...
                    if remaining_plan is None:
                        failure = "fok_depth"
                        break
                    guaranteed_edge = 1 - (execution_cost + remaining_plan[0]) / shares
                    if guaranteed_edge <= args.min_edge:
                        failure = "edge_revalidation"
                        break
                    current_fill = next(
                        row for row in remaining_plan[2] if row["token"] == token
                    )
                    ...
                    hard_base.append_csv(
                        ...
                        ["timestamp", "event_id", "action", "token", "shares", "price", "fee", "cost", "detail"],
                        {
                            ...
                            "cost": leg_cost,
                            "detail": f"leg={index + 1}/{len(order)} edge={guaranteed_edge:.8f}",
                        },
                    )
                if failure:
                    sequential_aborts += 1
                    residual, received, pnl = _unwind(
                        clob, filled, fee_map, args.slippage_bps
                    )
                    cash += received
                    realized += pnl
                    if residual:
                        aborting[event_id] = {
                            "shares": shares,
                            "cost": sum(float(row["cost"]) for row in residual),
                            "legs": residual,
                            "reason": failure,
                            "opened_ts": now,
                        }
                ...
                open_bundles[event_id] = {
                    ...
                    "cost": execution_cost,
                    "net_edge": edge,
                    "raw_edge": 1 - sum(float(row["raw_vwap"]) for row in filled),
                    "opened_ts": now,
                    "legs": len(markets),
    ...
    hard_base.atomic_json(state_path, out)
    hard_base.atomic_json(args.run_dir / "status.json", out)
    print(
...
def _self_test() -> int:
    ...
    assert fill.raw_vwap > 0.50 and fill.all_in_unit_price > fill.stressed_vwap
    ...
    print("v7_hard_arb_execution_self_test=ok")
    return 0
