...
SNAPSHOT_SCHEMA = "polymarket_v7_multi_crypto_feature_snapshot_v1"
...
class SegmentedWriter:
    def __init__(self, path: Path, segment_bytes: int):
        self.path = path; self.segment_bytes = segment_bytes; self.segment = 0
    ...
    def append(self, value: dict[str, Any]) -> None:
        ...
        if self.bytes_written and self.bytes_written + len(encoded) > self.segment_bytes:
            ...
            self.segment += 1; os.replace(self.path, sealed); self.bytes_written = 0
            self.handle = self.path.open("a", encoding="utf-8", buffering=1024 * 1024)
        self.handle.write(payload); self.bytes_written += len(encoded)

    def flush(self, durable: bool = False) -> None:
        self.handle.flush()
        if durable: os.fsync(self.handle.fileno())

    def close(self) -> None:
        self.flush(True); self.handle.close()
...
def main() -> int:
    ...
    try:
        while True:
            now=time.time_ns(); snapshot=load(args.snapshot)
