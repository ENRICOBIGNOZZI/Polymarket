...
class LedgerContractError(ValueError):
    """Raised when an execution-ledger record violates the V7 contract."""


class LedgerOwnershipError(RuntimeError):
    """Raised when a second writer attempts to own the canonical ledger."""
...
class LedgerEvent:
    ...
    def validate(self) -> None:
        ...
        if self.bid is not None and not (0.0 <= self.bid <= 1.0):
            raise LedgerContractError("bid:out_of_range")
        if self.ask is not None and not (0.0 <= self.ask <= 1.0):
            raise LedgerContractError("ask:out_of_range")
...
class CanonicalLedgerWriter:
    """Fail-closed single owner for the append-only V7 PAPER execution ledger."""
    ...
    def append(self, event: LedgerEvent) -> None:
        ...
        if event.model_sha != self.model_sha:
            raise LedgerContractError("model_sha:mixed_sha")
...
def iter_events(path: Path, *, expected_model_sha: str | None = None) -> Iterator[LedgerEvent]:
    if expected_model_sha is not None and not _SHA_RE.fullmatch(expected_model_sha):
    ...
    with Path(path).open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if expected_model_sha is not None and event.model_sha != expected_model_sha:
                raise LedgerContractError(f"line_{line_number}:mixed_sha")
            yield event


def load_events(path: Path, *, expected_model_sha: str | None = None) -> list[LedgerEvent]:
