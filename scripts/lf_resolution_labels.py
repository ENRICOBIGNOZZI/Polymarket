...
def fetch_resolution(
    ...
    *,
    ...
    if market is None or market.resolved_outcome not in (0, 1):
        return None
    timestamp = int(observed_ts if observed_ts is not None else time.time())
    return {
        "schema": ext.PRICE_SCHEMA,
        "observed_ts": timestamp,
        "observed_utc": ext.iso_utc(timestamp),
...
def backfill_resolution_labels(
    ...
    *,
    ...
    for market_id in before["missing_market_ids"][: max(0, int(max_markets))]:
        try:
            label = fetch_resolution(market_id, observed_ts=now, resolver=resolver)
        except Exception as exc:  # fail closed per research label request
