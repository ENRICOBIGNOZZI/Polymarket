...
SCHEMA = "polymarket_v7_external_bridge_status_v2"


def fetch_text(url: str, timeout: int = 20) -> str:
    request = urllib.request.Request(url, headers={"User-Agent": "polymarket-v7-paper/2"})
...
def approved_direct_candidate(report: dict[str, Any]) -> dict[str, Any] | None:
    """Return the exact integration-approved direct-probability candidate, if any.

    Authorization is intentionally candidate-level.  Never collapse an approved
    candidate to just (source, feature_name): two models from the same source can
    have different horizons/specifications and only the exact approved identity
    may authorize a PAPER probability.
    """
...
def candidate_provenance(candidate: dict[str, Any]) -> str:
    ...
    # Keep the historical five-column runtime CSV schema.  Immutable candidate
    # identity is carried inside provenance so existing PAPER readers remain
    # compatible while authorization remains exact and auditable.
...
def materialize(
    ...
    *,
    ...
    if candidate is not None:
        approved_source = str(candidate.get("source") or "")
        ...
        for raw in signal_text.splitlines():
            ...
            if not isinstance(row, dict):
                continue
            if str(row.get("source") or "") != approved_source:
                continue
...
def main() -> int:
    ...
    # Revoke any prior probability before *any* remote I/O.  A network failure,
    # crash, stale report or authorization miss must leave safe abstention.
