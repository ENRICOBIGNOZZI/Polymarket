...
def get_json(url:str,payload:Any|None=None,timeout:int=20)->Any:
    data=None if payload is None else json.dumps(payload).encode();req=urllib.request.Request(url,data=data,headers={"User-Agent":"polymarket-v6-hard-arb-paper/2","Content-Type":"application/json"})
...
def discover_event_ids(gamma:str,limit:int,min_liq:float)->list[str]:
    ...
    while len(ids)<limit and offset<5000:
        qs=urllib.parse.urlencode({"active":"true","closed":"false","limit":100,"offset":offset,"order":"liquidityNum","ascending":"false"});raw=get_json(gamma+"/markets?"+qs);batch=raw if isinstance(raw,list) else raw.get("markets",[]) if isinstance(raw,dict) else []
        if not batch:break
        for m in batch:
            ...
            eid=str(m.get("eventId") or "");ev=m.get("events")
...
def event_spec(gamma:str,event_id:str)->list[dict]|None:
    ...
    for m in ms:
        ...
        if m.get("closed") or m.get("active") is False or m.get("enableOrderBook") is False or m.get("acceptingOrders") is False:return None
        clean.append(m)
    return clean
...
def main()->int:
    ...
    locked_cost=sum(float(b["cost"]) for b in open_b.values());locked_profit=sum(float(b["shares"])-float(b["cost"]) for b in open_b.values());equity=cash+locked_cost;peak=max(peak,equity);dd=max(0.0,1-equity/peak) if peak else 0.0;killed=bool(state.get("killed")) or dd>=max_dd
    ...
    if not killed:
        ...
        for eid in eids:
            ...
            try:
                ms=event_spec(gamma,eid)
                if ms is None:continue
                tokens=[];rates=[]
                for m in ms:
                    yes,_=market_tokens(m) or ("","");tokens.append(yes);fs=m.get("feeSchedule") if isinstance(m.get("feeSchedule"),dict) else {};rates.append((max(0.0,finite(fs.get("rate"),.07)),max(0.0,finite(fs.get("exponent"),1.0))))
                ...
                min_size=min(bs[t]["size"] for t in tokens);min_order=max(bs[t]["min_order"] for t in tokens);eq=max(1.0,equity);room=min(args.max_trade_usd,max(0.0,max_gross*eq-locked_cost),max(0.0,max_event*eq),cash);shares=min(min_size,room/max(cost_ps,1e-9))
                ...
                cash-=cost;open_b[eid]={"shares":shares,"cost":cost,"net_edge":edge,"raw_edge":1-raw,"opened_ts":now,"legs":len(ms)};locked_cost+=cost;locked_profit+=shares-cost;equity=cash+locked_cost;entered+=1
    ...
    state={"timestamp":now,"cash":cash,"equity":equity,"peak":peak,"drawdown":dd,"killed":killed,"bundles":open_b,"gross_exposure":locked_cost,"open_positions":len(open_b),"realized_pnl":realized,"locked_expected_profit":locked_profit,"scanned_events":scanned,"positive_candidates":candidates,"entered":entered,"best_edge":best_edge,"failures":failures,"paper_only":True,"atomic_snapshot_assumption":True,"marking":"cost_basis_until_resolution"};atomic_json(sp,state);atomic_json(args.run_dir/"status.json",state)
    append_csv(args.run_dir/"equity.csv",["timestamp","cash","equity","drawdown","gross_exposure","open_positions","locked_expected_profit","scanned_events","positive_candidates","entered","best_edge"],state)
    print(json.dumps({k:state[k] for k in ("equity","drawdown","open_positions","locked_expected_profit","scanned_events","positive_candidates","entered","best_edge","killed")},sort_keys=True));return 0
