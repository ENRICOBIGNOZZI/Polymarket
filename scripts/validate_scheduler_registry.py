...
CONTEXT_ACTIVE_IDS = {
    "administrator-supervisor", "research-policy", "research-queue", "integration-merge",
    "post-merge-validation", "paper-server-deploy", "paper-server-health",
    "forward-maker-research", "fast-arb-shadow-research", "arb-theory-research",
}
ALLOWED_CONTEXT_PROFILES = {"supervisor", "policy", "research", "integration", "validation", "remote", "api"}
...
def validate(root: Path, registry_path: Path) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    if not isinstance(administrator, dict):
        ...
        if administrator.get("scheduler_context_documentation") != "docs/SCHEDULER_CONTEXT.md":
            errors.append("administrator.scheduler_context_documentation must select docs/SCHEDULER_CONTEXT.md")
        ...
        if context_path.is_file():
            try:
                ...
                errors.extend(f"scheduler context: {error}" for error in module.validate_context(context_data))
                raw_assignments = context_data.get("scheduler_contract", {}).get("assignments", {})
                if isinstance(raw_assignments, dict):
                    context_assignments = {str(key): str(value) for key, value in raw_assignments.items()}
                else:
    ...
    required_fields = {
        "id", "workflow", "workflow_name", "job", "cadence", "responsibility", "context_profile",
    ...
    for index, raw in enumerate(schedulers):
        ...
        if context_assignments.get(scheduler_id) != context_profile:
            errors.append(f"scheduler context profile mismatch for {scheduler_id}: registry={context_profile!r} context={context_assignments.get(scheduler_id)!r}")
        ...
        if scheduler_id in CONTEXT_ACTIVE_IDS:
            ...
            if f"--scheduler-id {scheduler_id}" not in text:
                errors.append(f"{scheduler_id} does not identify its context profile during validation")
    ...
    if set(context_assignments) != ids:
        ...
        if missing_context:
            errors.append("scheduler context is missing registered ids: " + ", ".join(missing_context))
        if stale_context:
            errors.append("scheduler context has stale ids: " + ", ".join(stale_context))

    workflow_dir = root / ".github" / "workflows"
    actual_workflows = {str(path.relative_to(root)) for path in workflow_dir.iterdir()
    ...
    if admin:
        ...
        for forbidden in ("gh pr merge", "gh workflow run", "repository_dispatch", "POLYMARKET_DEPLOY_REF=", "git push origin paper-validated"):
    ...
    if integration:
        ...
        for required in ("BASE_MAIN_SHA", "BASE_VALIDATED_SHA", "candidate-final.json", "--match-head-commit",
                         "current_main_after_merge", '"event_type": "champion-integration-merged"'):
...
def render_report(items: list[dict[str, Any]], errors: list[str]) -> str:
    lines = ["# Scheduler registry validation", "", f"- schedulers: {len(items)}", f"- errors: {len(errors)}", "",
             ...
             "|---|---|---|---|---|---:|---:|---:|"]
    for item in items:
        lines.append("| {id} | `{profile}` | `{job}` | {cadence} | {responsibility} | {merge} | {deploy} | {dispatch} |".format(
            id=item["id"], profile=item["context_profile"], job=item["job"],
            cadence=str(item["cadence"]).replace("|", "/"), responsibility=str(item["responsibility"]).replace("|", "/"),
            merge="yes" if item["merge_authority"] else "no", deploy="yes" if item["deploy_authority"] else "no",
            dispatch="yes" if item["validation_dispatch_authority"] else "no"))
    if errors:
        lines.extend(["", "## Errors"] + [f"- {error}" for error in errors])
    else:
        lines.extend(["", "Registry, shared context and one-job-per-workflow contracts are valid."])
    return "\n".join(lines) + "\n"
