...
class PaperRouter:
    ...
    def compact_durable_evidence(self) -> None:
        ...
        with nullcontext() if journal else self.durable_counterfactual_path.open('ab') as handle:
            for _,row in index.iter_records(root_rank_gt=0,chronological=True):
                if journal:journal.append(row)
    ...
    def wait_for_exploration_receipt(
        ...
        # The dedicated V7_GLOBAL_PORTFOLIO_COORDINATOR daemon is the sole
        # consumer of opportunities/inbox. The router only waits for its
        # receipt; directly invoking process_cut here would create two inbox
        # consumers and can race the daemon while archiving the same file.
