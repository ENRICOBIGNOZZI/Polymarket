...
def _table(rows: list[dict[str, Any]], key: str) -> list[str]:
    out = [f"| {key} | orders | reachable | pess opp | fills | mean rest ms | near miss | resets |",
           "|---|---:|---:|---:|---:|---:|---:|---:|"]
    for row in rows[:20]:
        out.append(
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    stages = (
        ...
        ("Full fills", "full_fills"), ("Cancelled before flow", "cancelled_before_flow"), ("Priority resets", "priority_resets"),
    )
    ...
    lines.extend(["", "## Closest zero-fill orders", ""])
    ...
    lines.extend([
        "## Evidence caveat",
        "",
        f"The independent trade tape has `{quality.get('timestamp_resolution_ms',1000)} ms` exchange timestamp resolution. "
        "Same-second ambiguous trades are rejected and the tape is not simulator truth. A simulator-bug claim requires exact WebSocket episode replay.",
        "",
    ])
    return "\n".join(lines)
