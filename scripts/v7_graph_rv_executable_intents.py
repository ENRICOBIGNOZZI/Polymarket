...
def scan(cfg: dict[str, Any], now: int, *, shared_state: Path | None = None,
    ...
    capital_scope = cfg.get("capital_scope") if isinstance(cfg.get("capital_scope"), dict) else {}
    max_notional = max(0.0, float(capital_scope.get("execution_budget", 0.0)))
