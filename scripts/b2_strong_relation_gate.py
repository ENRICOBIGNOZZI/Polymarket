...
def certificate_is_strong(cert: Certificate, min_jaccard: float, min_shared_tokens: int) -> bool:
...
def classify_row(
    ...
    certs = [x for x in (row.get("coherence_scope") or "").split("|") if x]
    hedge_ids = parse_hedge_ids(row)
    if not certs:
        return False, ["missing_relation_certificate"], []
    if len(certs) != len(hedge_ids):
        return False, ["certificate_hedge_count_mismatch"], []

    target = metadata.get((row.get("market") or "").strip())
    weak: list[str] = []
    notes: list[str] = []
    for raw, hedge_id in zip(certs, hedge_ids):
        cert = parse_certificate(raw)
        if cert is None or not certificate_is_strong(cert, min_jaccard, min_shared_tokens):
            weak.append(raw)
            continue
        if cert.scope == "same_event":
            notes.append(f"{hedge_id}:same_event")
            continue
        hedge = metadata.get(hedge_id)
        if not target or not hedge:
            weak.append(raw)
            notes.append(f"{hedge_id}:context=<missing>")
            continue
        shared = sorted(meaningful_context_tokens(target) & meaningful_context_tokens(hedge))
        notes.append(f"{hedge_id}:context={','.join(shared) if shared else '<none>'}")
        if len(shared) < min_context_shared_tokens:
            weak.append(raw)
    return not weak, weak, notes
...
def gate_rows(
    ...
    for row in rows:
        ok, weak, notes = classify_row(
            row, metadata, min_jaccard, min_shared_tokens, min_context_shared_tokens
        )
        item = dict(row)
...
def main() -> int:
    ...
    kept, rejected = gate_rows(
        rows, metadata, args.min_jaccard, args.min_shared_tokens, args.min_context_shared_tokens
    )
    base_fields = fieldnames or ["coherence_scope"]
    output_fields = base_fields + [x for x in ["strong_relation_context"] if x not in base_fields]
    rejection_fields = output_fields + [
        x for x in ["strong_relation_reason", "weak_relation_certificates"] if x not in output_fields
    ]
    write_rows(args.output, output_fields, kept)
    write_rows(args.rejections, rejection_fields, rejected)

    report = {
        "input_rows": len(rows), "kept_rows": len(kept), "rejected_rows": len(rejected),
        "metadata_rows": len(metadata), "min_jaccard": args.min_jaccard,
    ...
    print(
        f"b2_strong_relation_gate input={len(rows)} kept={len(kept)} rejected={len(rejected)} "
        f"metadata={len(metadata)} min_jaccard={args.min_jaccard:.2f} "
        f"min_shared={args.min_shared_tokens} min_context_shared={args.min_context_shared_tokens}"
    )
    return 0
