...
def resolve(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(profiles, dict) or not isinstance(rows, list) or len(rows) != 26:
    ...
    if len(actual_logs) != 24 or len(set(actual_logs)) != 24 or set(actual_logs) != declared_logs:
