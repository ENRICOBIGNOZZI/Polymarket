...
def rep(path: str, old: str, new: str) -> None:
    ...
    count = s.count(old)
    if count != 1:
        raise SystemExit(f"anchor count {path}: {count}")
...
Path("include/pm/rx_thread_alignment.hpp").write_text(
    '''#pragma once
...
rep(
    "src/v7_crypto_flash_shadow_runtime.cpp",
    "    int socket_busy_poll_us = 0;\n    int cpu_pin = 1;",
...
Path("tests/test_v7_dynamic_rx_alignment.py").write_text(
    '''from pathlib import Path
...
Path("docs/V7_DYNAMIC_RX_AFFINITY.md").write_text(
