...
def validate_tape_record(v: dict[str,Any], model_sha: str|None=None) -> dict[str,Any]:
    ...
    decision=int(v.get("decision_wall_ns") or 0); available=int(v.get("available_at_ns") or 0)
    if decision<=0 or available<=0 or available>decision: raise ValueError("tape availability invalid")
...
def label_rows(rows: list[dict[str,Any]], policy: dict[str,Any]) -> tuple[list[dict[str,Any]],dict[str,Any]]:
    ...
    for row in rows: groups.setdefault(str(row.get("market_id") or ""),[]).append(row)
    ...
    for market_id, seq in groups.items():
        ...
        for i,origin in enumerate(seq):
            ...
            for h in policy["horizons_ms"]:
                ...
                if end_ns>0 and target>=end_ns: status="CONTRACT_EXPIRES_BEFORE_HORIZON"
                elif p0 is None or not book0: status="ORIGIN_BOOK_INVALID"
                else:
                    ...
                    if j<=i: status="NO_LATER_ASOF_SAMPLE"
                        ...
                        elif ff.get("pm_book_valid") is not True or p1 is None: status="TARGET_BOOK_INVALID"
                        else:
