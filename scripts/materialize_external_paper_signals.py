...
def candidate(row: dict[str, Any], *, now: int, max_age_seconds: int) -> dict[str, Any] | None:
...
def materialize(rows: Iterable[dict[str, Any]], *, now: int, max_age_seconds: int) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    output = materialize(rows, now=now, max_age_seconds=args.max_age_seconds)
