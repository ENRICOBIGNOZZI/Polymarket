...
NativeProbabilityModel NativeProbabilityModel::load(
    ...
    const auto chase_ticks=o.at("maximum_chase_ticks").as_int64();
    if(chase_ticks<0 || chase_ticks>20)
        throw std::invalid_argument("probability experiment chase bound");
    model.maximum_chase_ticks=static_cast<std::int32_t>(chase_ticks);
