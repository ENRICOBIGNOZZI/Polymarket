...
bool CoinbaseL2Book::top_of_book(CoinbaseDepthLevel& bid, CoinbaseDepthLevel& ask) const noexcept {
    if (state_ != CoinbaseL2State::Live || !uncrossed()) return false;
    bid = CoinbaseDepthLevel{storage_->bids.begin()->first, storage_->bids.begin()->second};
    ask = CoinbaseDepthLevel{storage_->asks.begin()->first, storage_->asks.begin()->second};
    return true;
}
