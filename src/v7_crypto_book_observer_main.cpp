...
Options parse(int argc, char** argv) {
    ...
    if (out.collector_sha.empty() || out.runtime_sha.empty()) {
        throw std::runtime_error("--collector-sha and --runtime-sha are required");
    }
    return out;
}
} // namespace
...
int main(int argc, char** argv) {
    try {
        ...
        pm::v7::research::CryptoBookObserver observer(
            options.run_root, options.config, options.collector_sha,
            options.runtime_sha, options.ws_url);
