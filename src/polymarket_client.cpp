...
std::unordered_map<std::string, TokenMeta> PolymarketClient::discover_tokens() const {
    ...
    for (std::size_t offset = 0; offset < cfg_.max_markets; offset += page) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            std::string event_id, event_title, event_category;
            ...
            if (json_object_object_get_ex(m, "events", &events) && events && json_object_is_type(events, json_type_array) && json_object_array_length(events) > 0) {
                ...
                event_id = jstr(e0, "id"); event_title = jstr(e0, "title"); event_category = jstr(e0, "category");
            }
            ...
            for (std::size_t k = 0; k < token_ids.size(); ++k) {
                TokenMeta t;
                t.token_id = token_ids[k]; t.market_id = market_id; t.condition_id = condition_id;
                t.event_id = event_id.empty() ? market_id : event_id; t.event_title = event_title;
                t.question = question; t.slug = jstr(m, "slug"); t.category = category; t.outcome = outcomes[k];
                t.end_date = jstr(m, "endDate"); t.active = true; t.closed = false; t.neg_risk = neg_risk;
                t.enable_order_book = true; t.liquidity = liquidity; t.volume = volume; t.reference_price = prices[k];
    ...
    for (std::size_t start = 0; start < ids.size(); start += cfg_.book_batch_size) {
        ...
        try {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                BookSnapshot snap;
                snap.token_id = token; snap.market_id = jstr(b, "market");
                snap.bids = parse_levels(b, "bids"); snap.asks = parse_levels(b, "asks");
                snap.last_trade = jdouble(b, "last_trade_price", std::numeric_limits<double>::quiet_NaN());
                snap.tick_size = jdouble(b, "tick_size", 0.01); snap.min_order_size = jdouble(b, "min_order_size", 1.0);
                snap.neg_risk = jbool(b, "neg_risk", false); out[token] = std::move(snap);
            }
        } catch (const std::exception& e) { std::cerr << "book batch failed: " << e.what() << '\n'; }
    }
    return out;
}

std::unordered_map<std::string, std::vector<HistoricalPoint>> PolymarketClient::fetch_warm_history(
        const std::unordered_map<std::string, TokenMeta>& tokens) const {
    ...
    std::unordered_map<std::string, std::vector<HistoricalPoint>> out;
    ...
    for (std::size_t start = 0; start < ranked.size(); start += batch) {
        ...
        json_object* arr = json_object_new_array();
        for (std::size_t i = start; i < stop; ++i) json_object_array_add(arr, json_object_new_string(ranked[i].second.c_str()));
        ...
        try {
            ...
            if (!root || !json_object_object_get_ex(root.get(), "history", &history) || !history || !json_object_is_type(history, json_type_object)) continue;
            json_object_object_foreach(history, key, arr_hist) {
                if (!arr_hist || !json_object_is_type(arr_hist, json_type_array)) continue;
                std::vector<HistoricalPoint> px;
                ...
                for (int i = 0; i < n; ++i) {
                    ...
                    const auto ts = static_cast<std::int64_t>(jdouble(point, "t", 0.0));
                    if (ts > 0 && std::isfinite(p) && p > 0.0 && p < 1.0) px.push_back({ts, p});
                }
                std::sort(px.begin(), px.end(), [](const auto& a, const auto& b){ return a.ts < b.ts; });
                if (!px.empty()) out[key] = std::move(px);
            }
        } catch (const std::exception& e) { std::cerr << "history batch failed: " << e.what() << '\n'; }
    }
    return out;
}
