...
Gamma exposes ``feeSchedule`` on market objects and the CLOB market descriptor
exposes the same values in compact ``fd`` form.  Those per-market descriptors
are the source of truth: category-wide constants and ``/fee-rate`` signing
metadata are deliberately not used as P&L fallbacks.
"""
