...
class Ranking15mRegistrationTest(unittest.TestCase):
    ...
    def test_frozen_cutoff_and_history_anchor_use_15m_grid(self) -> None:
        cfg = self.cfg
        holdout = int(cfg["discovery"]["forward_holdout_start_ts"])
        bucket_seconds = 15 * 60
        embargo_seconds = int(cfg["history"]["purge_embargo_buckets"]) * bucket_seconds
        self.assertEqual(holdout - embargo_seconds, 1787787900)
        start, required = frozen.history_start_for_frozen_fit(
            now_ts=holdout,
            holdout_start_ts=holdout,
            rolling_lookback_seconds=int(cfg["history"]["lookback_hours"]) * 3600,
            training_window_seconds=int(cfg["history"]["training_window_days"]) * 86400,
            bucket_seconds=bucket_seconds,
        )
        self.assertLessEqual(start, required)
        self.assertEqual(required, holdout - 21 * 86400 - 12 * bucket_seconds)
