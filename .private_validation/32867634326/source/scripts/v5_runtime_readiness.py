#!/usr/bin/env python3
"""Validate V5 paper runtime without conflating startup liveness and completed model output."""
...
def evaluate(
    ...
    *,
    ...
    for field, label in (
        ("recorder_alive", "trade recorder"),
        ("broker_alive", "multi-leg broker"),
        ("allocator_alive", "allocator"),
    ):
        if supervisor.get(field) != "1":
            raise ReadinessError(f"{label} is not alive")
