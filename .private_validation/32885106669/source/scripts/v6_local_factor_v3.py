...
def unit_root_block_pvalue(residual: list[float], seed: int, reps: int = 300) -> tuple[float, float]:
    """One-sided dependence-aware bootstrap that imposes the unit-root null.

    The bootstrap resamples centered increments in circular blocks, reconstructs
    level paths with gamma=0/phi=1, and recomputes the ADF-style statistic. BH/FDR
    is applied only after these marginal p-values exist.
    """
    if len(residual) < 32:
        return 1.0, 0.0
    ...
    n = len(centered)
    if n < 2 or max(centered, default=0.0) - min(centered, default=0.0) <= 1e-12:
        return 1.0, observed
    ...
    return (left + 1) / (total + 1), observed
...
def guarded_hold_seconds(
    ...
    *,
    ...
    """Fail closed unless every leg has a pre-resolution exit window."""
    if not phis or len(end_times) != len(phis) or any(end is None for end in end_times):
        return None
    bar_seconds = max(60, int(fidelity_minutes) * 60)
    half_lives = [half_life_bars(phi) for phi in phis]
    if any(not math.isfinite(value) for value in half_lives):
        return None
    nominal = max(float(bar_seconds), min(24.0 * 3600.0, 2.0 * max(half_lives) * bar_seconds))
    room = min(int(end) for end in end_times if end is not None) - int(now) - max(0, int(exit_buffer_seconds))
    if room < bar_seconds:
        return None
    return min(nominal, float(room))
...
def build_pair(
    ...
    *,
    ...
    best: tuple[float, float, Any, Any, float, float] | None = None
    for i, a in enumerate(signals):
        for b in signals[i + 1:]:
            ...
            for signal, _side, _weight in legs:
                ...
                end_times.append(market_end_ts(raw))
                phis.append(signal.phi)
            if not valid:
                continue
            hold_seconds = guarded_hold_seconds(
                phis,
                end_times,
                now=now,
                fidelity_minutes=fidelity_minutes,
                exit_buffer_seconds=exit_buffer_seconds,
            )
            ...
            capital = 0.0
            expected_pnl = 0.0
            leg_state: list[tuple[Any, str, float, Any, str]] = []
            units = math.inf
            min_units = 0.0
            for signal, side, weight in legs:
                ...
                entry_fee = fee_per_share(book.bid, fee, taker=False)
                exit_fee = fee_per_share(exit_price, fee, taker=True)
                pnl = exit_price - exit_fee - book.bid - entry_fee
                if pnl <= 0.0:
                    valid = False
                    break
                capital += weight * (book.bid + entry_fee)
                expected_pnl += weight * pnl
                units = min(units, book.bid_size / weight)
                min_units = max(min_units, book.min_order / weight)
                leg_state.append((signal, side, weight, book, token))
            if not valid or capital <= 0.0 or not math.isfinite(units) or units + 1e-12 < min_units:
                continue
            edge = expected_pnl / capital
            if edge <= min_edge:
                continue
            max_notional = min(max_trade, units * capital)
            if max_notional <= 0.0:
                continue
            scale = max_notional / capital
            fill_probs: list[float] = []
            for _signal, _side, weight, book, token in leg_state:
                target_shares = max(book.min_order, scale * weight)
                ...
                fill_probs.append(
                    fill_probability_proxy(
                        ...
                        own_shares=target_shares,
                        compatible_flow_per_second=rate,
                        horizon_seconds=min(180.0, hold_seconds),
                        prior_flow_per_second=1.0 / 300.0,
                    )
                )
            per_leg_floor = min(fill_probs, default=0.0)
            if per_leg_floor < min_fill_probability:
                continue
            utility = edge * per_leg_floor
            if best is None or (utility, edge) > (best[0], best[1]):
                best = (utility, edge, a, b, hold_seconds, max_notional)
    if best is None:
        return [], "no_fillable_pair"

    _, edge, a, b, hold_seconds, max_notional = best
    side_a = "NO" if a.residual_z > 0 else "YES"
    side_b = "NO" if b.residual_z > 0 else "YES"
    sign_a = -1.0 if side_a == "NO" else 1.0
    sign_b = -1.0 if side_b == "NO" else 1.0
    weight_b = abs((sign_a * a.loading) / (sign_b * b.loading))
    legs = [(a, side_a, 1.0), (b, side_b, weight_b)]
    bundle = f"LOCAL_FACTOR-V3-{now}-{serial}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V6 repaired local-factor paper research challenger")
    ...
    eligible = {
        ...
        if cutoff > 0.0 and candidate.pvalue <= cutoff
    }
    ...
    status = {
        ...
        "horizon": "n_step_actual_fidelity_ttr_guarded",
        "ttr_missing_policy": "fail_closed",
        "tape_clock": "received_ms_with_exchange_timestamp_fallback",
        "fill_proxy_size": "actual_target_shares",
        "profile": {
