...
# The proxy cache is verified discovery metadata only; order books and trades
# are still fetched live.  Keep this aligned with v6_market_proxy.py's bounded
# one-hour fallback on validated main.
MARKET_PROXY_MAX_CACHE_AGE_SECONDS = 3600.0
...
def main() -> int:
    ...
    proxy_cache_age = f(proxy.get("cache_age_seconds"), MISSING_AGE_SECONDS)
    proxy_ready = (
        bool(proxy)
        and str(proxy.get("source") or "") not in {"", "startup", "unavailable"}
        and f(proxy.get("markets"), -1.0) > 0.0
        and 0.0 <= proxy_cache_age <= MARKET_PROXY_MAX_CACHE_AGE_SECONDS
    )
    ...
    status = {
        ...
        "proxy_cache_age_seconds": proxy_cache_age,
