...
class GrafanaV6OperationsContractTests(unittest.TestCase):
    ...
    def test_native_status_matches_validated_discovery_cache_window(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            root=Path(tmp_dir); config=root/"paper_v6.json"; now=int(time.time())
            config.write_text((ROOT/"config/paper_v6.json").read_text(encoding="utf-8"),encoding="utf-8")
            self._write_runtime_inputs(root,now)
            proxy_path=root/"market_proxy_status.json"
            proxy_path.write_text(json.dumps({
                "timestamp":now,"source":"relay_cache","markets":300,"cache_age_seconds":1800,
            }),encoding="utf-8")
            command=[sys.executable,str(ROOT/"scripts/v6_runtime_status.py"),"--config",str(config),"--run-root",str(root)]
            subprocess.run(command,check=True,capture_output=True,text=True)
            runtime=json.loads((root/"runtime_status.json").read_text(encoding="utf-8"))
            self.assertTrue(runtime["proxy_ready"])
            self.assertEqual(runtime["proxy_cache_age_seconds"],1800)

            proxy_path.write_text(json.dumps({
                "timestamp":now,"source":"relay_cache","markets":300,"cache_age_seconds":3601,
            }),encoding="utf-8")
            subprocess.run(command,check=True,capture_output=True,text=True)
            runtime=json.loads((root/"runtime_status.json").read_text(encoding="utf-8"))
            self.assertFalse(runtime["proxy_ready"])
