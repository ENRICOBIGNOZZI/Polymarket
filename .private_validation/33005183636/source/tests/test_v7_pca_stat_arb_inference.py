...
def test_research_driver_uses_successor_contracts():
    source = (ROOT / "scripts" / "v7_pca_stat_arb_research.py").read_text(encoding="utf-8")
    assert "predeclare_target_controls" in source
    assert "conditional_target_bootstrap_pvalue" in source
    assert "benjamini_yekutieli_selected" in source
    assert "score_with_total_single_leg_risk" in source
    assert "resolve_fee_details" in source
    assert "uncertainty_penalty" in source
    assert "unestimable_pvalue\": 1.0" in source
