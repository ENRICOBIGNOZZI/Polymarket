...
class V6ExecutionMeasurementTest(unittest.TestCase):
    ...
    def test_exploration_skips_second_side_without_stopping_later_markets(self) -> None:
        ...
        def book(token: str, depth: float) -> object:
            return qf.micro_legacy.Book(
                {
                    "asset_id": token,
                    "tick_size": "0.01",
                    "min_order_size": "1",
                    "bids": [{"price": "0.45", "size": str(depth)}],
                    "asks": [{"price": "0.55", "size": str(depth)}],
                }
            )

        markets = [market("m1"), market("m2")]
        books = {
            token: book(token, 30.0 if token.startswith("m1") else 4.0)
            for item in markets
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with tape.open("w", newline="", encoding="utf-8") as handle:
                ...
                writer.writeheader()
                for item, repetitions in ((markets[0], 2), (markets[1], 1)):
                    for token in (item.yes, item.no):
