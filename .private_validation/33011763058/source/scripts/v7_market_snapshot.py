...
import v7_market_proxy_base as proxy_base
from v7_market_proxy import Proxy, request_json

FALLBACK_MARKETS = proxy_base.FALLBACK_MARKETS
valid_cache_market = proxy_base.valid_cache_market
PAGE_SIZE = 100
PAGE_ATTEMPTS = 3
...
def _valid_liquid_row(row: Any, min_liquidity: float) -> bool:
    return valid_cache_market(row) and number(row.get("liquidityNum"), -1.0) >= max(0.0, min_liquidity)


def _gamma_page(gamma: str, offset: int, limit: int, min_liquidity: float) -> tuple[int, list[dict[str, Any]], list[str]]:
    errors: list[str] = []
    params = {
        "active": "true", "closed": "false", "limit": str(max(1, min(PAGE_SIZE, limit))),
        "offset": str(max(0, offset)), "order": "liquidityNum", "ascending": "false",
        "liquidity_num_min": str(max(0.0, min_liquidity)),
    }
    url = gamma.rstrip("/") + "/markets?" + urllib.parse.urlencode(params)
    for attempt in range(PAGE_ATTEMPTS):
        try:
            value = request_json(url, timeout=PAGE_TIMEOUT_SECONDS)
            if not isinstance(value, list):
                raise RuntimeError("Gamma offset page is not a list")
            rows = [dict(row) for row in value if isinstance(row, dict) and _valid_liquid_row(row, min_liquidity)]
            if rows:
...
def _merge_rows(target: dict[str, dict[str, Any]], rows: list[dict[str, Any]], min_liquidity: float) -> None:
    for row in rows:
...
def build_fresh_rows(proxy: Proxy, requested: int, min_markets: int, min_liquidity: float) -> tuple[list[dict[str, Any]], str, list[str]]:
    merged: dict[str, dict[str, Any]] = {}
    errors: list[str] = []
    offsets = list(range(0, requested, PAGE_SIZE))
    with ThreadPoolExecutor(max_workers=min(3, max(1, len(offsets)))) as pool:
        futures = [pool.submit(_gamma_page, proxy.gamma, offset, min(PAGE_SIZE, requested - offset), min_liquidity) for offset in offsets]
    ...
    if len(merged) < requested:
        query = {
            ...
            "ascending": ["false"], "liquidity_num_min": [str(max(0.0, min_liquidity))],
        }
        try:
            _merge_rows(merged, proxy.gamma_rows(requested, query), min_liquidity)
            source = "gamma_offset_plus_keyset"
        except Exception as exc:
    ...
    rows = sorted(merged.values(), key=lambda row: number(row.get("liquidityNum"), 0.0), reverse=True)[:requested]
    if len(rows) < min_markets:
        raise RuntimeError(f"fresh V7 relay cache has only {len(rows)} markets; need {min_markets}; {'; '.join(errors[-12:])}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build a fresh V7 PAPER public-market cache for relay delivery")
    ...
    rows, source, errors = build_fresh_rows(proxy, requested, min_markets, max(0.0, args.min_liquidity))
    ...
    if not isinstance(payload, dict) or payload.get("schema") != CACHE_SCHEMA:
        raise RuntimeError("V7 relay cache schema is invalid")
    cache_rows = payload.get("markets")
    if not isinstance(cache_rows, list) or len(cache_rows) < min_markets:
        raise RuntimeError("V7 relay cache did not persist enough markets")
    cache_age = max(0.0, time.time() - number(payload.get("timestamp"), -1.0))
    if cache_age > max(0.0, args.max_cache_age_seconds):
        raise RuntimeError(f"V7 relay cache is too old: {cache_age:.1f}s")
    if not all(valid_cache_market(row) and number(row.get("liquidityNum")) >= max(0.0, args.min_liquidity) for row in cache_rows):
        raise RuntimeError("V7 relay cache contains invalid market rows")

    summary = {
        "schema": "polymarket_v7_market_cache_relay_v1",
