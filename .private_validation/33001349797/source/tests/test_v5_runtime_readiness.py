...
class V5RuntimeReadinessTest(unittest.TestCase):
    ...
    def _write_runtime(
        ...
        *,
        ...
        with (self.root / "strategy_status.csv").open("w", newline="", encoding="utf-8") as handle:
            writer = csv.DictWriter(
                handle,
                fieldnames=("timestamp", "name", "expert", "capital_fraction", "alive", "status_age_seconds"),
            )
            writer.writeheader()
        ...
        with (self.root / "allocator_events.csv").open("w", newline="", encoding="utf-8") as handle:
            writer = csv.DictWriter(
                handle,
                fieldnames=("timestamp", "strategy", "expert", "event", "restart_count", "detail"),
            )
            writer.writeheader()
            for name in sorted(EXPECTED_MODELS):
                writer.writerow(
    ...
    def test_observed_missing_first_status_is_ready_only_during_startup_grace(self) -> None:
    ...
    def test_missing_first_status_after_startup_grace_fails_closed(self) -> None:
