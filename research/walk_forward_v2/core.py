...
def economic_evaluation(evaluations, *, latency_ms=(10, 25, 50, 100, 250, 500)):
    ...
    # Prespecified capital-based PAPER sizing sensitivity under exact current
    # live geometry. It changes only order notional and never selects a winner.
    ...
    for family in ("POOLED", "ASSET_SPECIFIC"):
        ...
        for horizon in (500, 1000, 2000):
            hkey = str(horizon)
            selector = (
                (lambda event, h=hkey: (event.get("markout_predictions", {}).get(h), None))
                if family == "POOLED"
                else (lambda event, h=hkey: (event.get("asset_markout_predictions", {}).get(h), None))
            )
            horizon_result = {}
