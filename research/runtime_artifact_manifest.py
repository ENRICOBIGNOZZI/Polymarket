...
def build(target_sha:str,maker:Path,rich:Path|None)->dict:
    ...
    value={'schema':SCHEMA,'version':1,'paper_only':True,'authenticated_execution':False,'real_order_submission':False,'target_model_sha':target_sha,
      'created_at_ns':time.time_ns(),'runtime_training':False,'maker_execution_model':{'path':'maker_execution_model.json','sha256':digest(maker),'model_state':m.get('model_state'),'policy_hash':m.get('policy_hash'),'config_hash':m.get('config_hash')},'rich_research_model':rich_meta}
    value['bundle_id']=hashlib.sha256(canonical({k:v for k,v in value.items() if k not in {'created_at_ns','bundle_id'}})).hexdigest()
    return value

def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument('--target-sha',required=True); ap.add_argument('--maker-model',type=Path,required=True); ap.add_argument('--rich-model',type=Path); ap.add_argument('--output',type=Path,required=True)
    a=ap.parse_args(); v=build(a.target_sha,a.maker_model,a.rich_model); a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(v,sort_keys=True,indent=2)+'\n'); print(json.dumps({'bundle_id':v['bundle_id'],'target_model_sha':v['target_model_sha'],'rich_model_state':v['rich_research_model']['state']},sort_keys=True)); return 0
