...
class DirectActionValueModel:
    ...
    def __init__(
        ...
        *,
        ...
        if not (
            0.0 <= self.minimum_observability_probability <= 1.0
        ):
            raise ValueError(
    ...
    def _iter_observability_rows(self, rows, *, markets=None):
        """Binary support target; missing economics remains censored elsewhere."""
        market_filter = set(markets) if markets is not None else None
        for row in rows:
            if market_filter is not None and str(row["market_id"]) not in market_filter:
                continue
    ...
    def _observability_probability(
        ...
        raw = float(model.predict(action))
        ...
        self.training_receipt = {
            ...
            **SAFETY,
            ...
            "minimum_observability_probability": (
                self.minimum_observability_probability),
            "observability_penalty_dollars": (
                self.observability_penalty_dollars),
    ...
    def score_actions(self, row, *, latency_ms=50, available_capital=None,
        ...
        if (
            observability_gate_candidates > 0
            and observability_gate_rejections
            == observability_gate_candidates
        ):
    ...
    return {
        ...
        **SAFETY,
        ...
        "total_observability_penalty": sum(
            float(row.get("observability_penalty") or 0.0)
            for row in trades),
