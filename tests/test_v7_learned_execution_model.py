...
def ev(kind, oid='', **kw):
    d=dict(event_type=kind,strategy='CRYPTO_RESEARCH_TEST_ENGINE',model_sha=SHA,paper_only=True,authenticated_execution=False,
...
def sub(oid,ts,q,*,bundle=None,leg=None,strategy='CRYPTO_RESEARCH_TEST_ENGINE',ids=None,count=2,imb=0.0):
    bd=100*(1+imb); md={}
...
def fill(oid,ts,qty=10.,*,fid=None,bundle=None,leg=None,strategy='CRYPTO_RESEARCH_TEST_ENGINE',token=None,complete=None):
...
def timeout(oid,ts,strategy='CRYPTO_RESEARCH_TEST_ENGINE'): return ev('ORDER_STATE',oid,strategy=strategy,recorded_ts_ms=ts,order_state='TIMEOUT')

def mark(oid,fill_ts,val,h='60s',*,fid=None,bundle=None,leg=None,strategy='CRYPTO_RESEARCH_TEST_ENGINE',token=None,delta=100):
...
def test_fill_and_markout_models_oos():
    ...
    fm=r['strategy_models']['CRYPTO_RESEARCH_TEST_ENGINE']['fill_model']; assert fm['state']=='OOS_SCORED' and fm['oos_brier']<.35*fm['baseline_brier']
    events=[]
    for i in range(320):
    ...
    mm=r['strategy_models']['CRYPTO_RESEARCH_TEST_ENGINE']['markout_models']['60s']; assert mm['state']=='OOS_SCORED' and mm['oos_rmse']<mm['baseline_rmse']
...
def test_joint_distribution_not_marginal_product_and_stratified():
    base=[JointExample(str(i),'CRYPTO_RESEARCH_TEST_ENGINE',('1','2'),2,i,i,(0.,)*4,'COMPLETE|COMPLETE' if i%2==0 else 'NO_FILL|NO_FILL') for i in range(200)]
    ...
    rows=[]
    for strategy,sig,off in [('CRYPTO_RESEARCH_TEST_ENGINE',('buy','sell'),0),('CRYPTO_RESEARCH_TEST_ENGINE',('left','right'),10000),('CRYPTO_SETTLEMENT_ENGINE',('buy','sell'),20000)]:
    ...
    r=joint_report(rows,1.,.25,0,40,15); assert set(r)=={'CRYPTO_RESEARCH_TEST_ENGINE::buy|sell','CRYPTO_RESEARCH_TEST_ENGINE::left|right','CRYPTO_SETTLEMENT_ENGINE::buy|sell'}

def test_strategy_stratification_and_fail_closed_report_contract():
    events=[]
    for strategy,rev,base in [('CRYPTO_RESEARCH_TEST_ENGINE',False,1000),('CRYPTO_SETTLEMENT_ENGINE',True,100000)]:
        for i in range(120):
    ...
    assert set(r['strategy_models'])=={'CRYPTO_RESEARCH_TEST_ENGINE','CRYPTO_SETTLEMENT_ENGINE'} and 'fill_model' not in r and r['research_only'] is True and r['decision']=='MORE_EVIDENCE_REQUIRED'
