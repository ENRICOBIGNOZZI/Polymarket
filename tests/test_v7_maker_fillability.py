...
class V7MakerFillabilityTest(unittest.TestCase):
    ...
    def _order(order_id: str = "o1", *, side: str = "BUY", queue: float = 5.0, limit: float = .50,
        ...
        return {
            "strategy": "CRYPTO_SETTLEMENT_ENGINE", "model_sha": SHA, "event_type": "ORDER_SUBMITTED",
            ...
            "decision_ts_ms": decision_ms, "metadata": {"component": "professional_maker", "model_family": "professional_maker", "action": "JOIN"},
        }

    @staticmethod
    def _state(order_id: str, state: str, ts_ms: int) -> dict:
        return {"strategy": "CRYPTO_SETTLEMENT_ENGINE", "model_sha": SHA, "event_type": "ORDER_STATE",
                "order_id": order_id, "order_state": state, "recorded_ts_ms": ts_ms, "metadata": {"component": "professional_maker"}}

    @staticmethod
    def _fill(order_id: str, size: float = 2.0) -> dict:
        return {"strategy": "CRYPTO_SETTLEMENT_ENGINE", "model_sha": SHA, "event_type": "FILL",
                "order_id": order_id, "filled_size": size, "recorded_ts_ms": 103_000, "metadata": {"component": "professional_maker"}}
