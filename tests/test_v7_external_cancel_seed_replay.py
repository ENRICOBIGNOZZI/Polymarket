...
def test_protocol_is_exactly_frozen_and_receive_time_gridded() -> None:
    ...
    assert trigger["grid_ms"] == 25 and trigger["shock_window_ms"] == 100
    assert abs(trigger["minimum_absolute_log_return_bp"] - 0.3) < 1e-12
    assert trigger["cooldown_ms"] == 250
    development = protocol["development_semantics"]
    assert development["overlap_warmup_ms"] == 300 and development["overlap_tail_ms"] == 1200
