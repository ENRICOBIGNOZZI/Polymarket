...
def test_secret_key_is_bound_to_expected_ethereum_address() -> None:
    compiler = shutil.which("c++")
    assert compiler
    if subprocess.run(["pkg-config", "--exists", "libsecp256k1"]).returncode != 0:
        pytest.skip("libsecp256k1 candidate dependency not installed")
    ...
    # Independently frozen from eth-account Account.from_key(1).
    assert EXPECTED == "0x7E5F4552091A69125d5DfCb7b8C2659029395Bdf"
