...
class CanonicalEconomicsTest(unittest.TestCase):
    def test_engine_action_attribution_requires_complete_provenance_dimensions(self) -> None:
        ...
        order_event = order(
            strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="maker-order", leg_id="YES",
            family="professional_maker", horizon=300, event_id="event-maker",
            market_id="market-1", extra_metadata=evidence,
        )
        fill_event = fill(
            strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="maker-order", fill_id="maker-fill",
            leg_id="YES", family="professional_maker", horizon=300,
            fee=0.01, event_id="event-maker", market_id="market-1",
            extra_metadata=evidence,
        )
        final_event = final(
            strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="maker-order",
            family="professional_maker", horizon=300,
    ...
    def test_inventory_merge_does_not_manufacture_a_market_fill(self) -> None:
        events = [
            ledger.LedgerEvent(
                event_type="INVENTORY_MERGE", strategy="CRYPTO_SETTLEMENT_ENGINE",
    ...
    def test_fee_aware_inventory_liquidation_closes_one_cost_complete_cycle(self) -> None:
        ...
        events = [
            ledger.LedgerEvent(
                event_type="INVENTORY_LIQUIDATION", strategy="CRYPTO_SETTLEMENT_ENGINE",
        ...
        self.assertEqual(
            list(report["strategy_day_stressed_net_pnl"]), ["CRYPTO_SETTLEMENT_ENGINE"]
        )
        self.assertAlmostEqual(
            sum(report["strategy_day_stressed_net_pnl"]["CRYPTO_SETTLEMENT_ENGINE"].values()),
            -0.164,
        )
        self.assertAlmostEqual(report["strategy_drawdown_usd"]["CRYPTO_SETTLEMENT_ENGINE"], 0.164)
        self.assertEqual(report["strategy_capacity_usd"]["CRYPTO_SETTLEMENT_ENGINE"], 25.0)

    def test_shadow_counterfactual_is_in_ledger_but_excluded_from_paper_pnl(self) -> None:
        now = clock()
        shadow = {
            "model_family": "CRYPTO_SETTLEMENT_ENGINE",
            "horizon_seconds": 300,
            "counterfactual": True,
    ...
    def test_crypto_taker_round_trip_is_one_mature_cost_complete_unit(self) -> None:
        now = clock()
        ...
        events.append(ledger.LedgerEvent(
            ...
            metadata={"realized": True, "unwind_accounted": True,
                      "cost_vector_complete": True,
                      "terminal_id": "crypto_settlement_engine:position:final"},
        ))
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
    ...
    def test_order_to_position_identity_is_one_mature_economic_unit(self) -> None:
        ...
        common = dict(
            ...
            side="BUY", metadata=metadata("CRYPTO_SETTLEMENT_ENGINE", 300),
        )
        events = [
            ledger.LedgerEvent(
            ...
            ledger.LedgerEvent(
                ...
                latency_cost=0.0, capital_duration_ms=300_000,
                metadata=metadata(
                    "CRYPTO_SETTLEMENT_ENGINE", 300, realized=True,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_events(path, events)
            report = econ.assess(
                path, expected_model_sha=SHA, family="CRYPTO_SETTLEMENT_ENGINE",
            )
        self.assertEqual(report["economic_units"], 1)
        self.assertEqual(report["submitted_units"], 1)
        self.assertEqual(report["complete_units"], 1)
    ...
    def test_order_to_multiple_positions_fails_closed(self) -> None:
        now = clock()
        order_event = order(
            strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="conflicted-order", leg_id="YES",
            family="crypto_informed_taker", horizon=300,
        )
        fills = [
            ledger.LedgerEvent(
                event_type="FILL", strategy="CRYPTO_SETTLEMENT_ENGINE", model_sha=SHA,
                ...
                exchange_ts_ms=now, receive_ts_ms=now + 1,
                metadata=metadata("crypto_informed_taker", 300),
            )
    ...
    def test_two_leg_completion_is_one_probability_bounded_unit_but_not_enough_evidence(self) -> None:
        ...
        events = [
            order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o-y", leg_id="YES", bundle_id="b1", required=required),
            order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o-n", leg_id="NO", bundle_id="b1", required=required),
            fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o-y", fill_id="f-y", leg_id="YES", bundle_id="b1", fee=0.02),
            fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o-n", fill_id="f-n", leg_id="NO", bundle_id="b1", fee=0.02),
            final(strategy="CRYPTO_SETTLEMENT_ENGINE", bundle_id="b1", pnl=1.0, slippage=0.01, unwind=0.0, capital=0.01, latency=0.01),
        ]
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
    ...
    def test_full_cost_vector_stress_can_flip_pnl_sign(self) -> None:
        events = [
            ...
            fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", fill_id="f1", leg_id="YES", family="crypto_informed_taker", horizon=30, fee=0.006),
            final(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", family="crypto_informed_taker", horizon=30, pnl=0.020, slippage=0.004, unwind=0.006, capital=0.004, latency=0.004),
        ]
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
    ...
    def test_missing_component_or_complete_vector_flag_fails_closed(self) -> None:
        events = [
            order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", leg_id="YES", family="crypto_informed_taker", horizon=30),
            fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", fill_id="f1", leg_id="YES", family="crypto_informed_taker", horizon=30, fee=0.0),
            final(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", family="crypto_informed_taker", horizon=30, pnl=1.0, slippage=0.0, unwind=0.0, capital=0.0, latency=None, cost_complete=False),
        ]
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
    ...
    def test_dynamic_family_and_horizon_are_not_silently_pooled(self) -> None:
        events = []
        for suffix, family, horizon, event_id in (("a", "professional_maker", 300, "event-a"), ("b", "crypto_informed_taker", 900, "event-b")):
            events.extend([
                order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=f"o-{suffix}", leg_id="YES", family=family, horizon=horizon, event_id=event_id),
                fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=f"o-{suffix}", fill_id=f"f-{suffix}", leg_id="YES", family=family, horizon=horizon, fee=0.0, event_id=event_id),
                final(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=f"o-{suffix}", family=family, horizon=horizon, pnl=1.0, slippage=0.0, unwind=0.0, capital=0.0, latency=0.0, event_id=event_id),
            ])
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
            mixed = econ.assess(path, expected_model_sha=SHA)
            maker = econ.assess(path, expected_model_sha=SHA, family="professional_maker", horizon_seconds=300)
        self.assertFalse(mixed["economic_evidence_ready"])
        ...
        self.assertFalse(maker["economic_evidence_ready"])
        self.assertIn("insufficient_distinct_event_clusters", maker["reason_codes"])
        self.assertEqual(maker["model_families_observed"], ["professional_maker"])
        self.assertEqual(maker["model_horizons_seconds_observed"], [300])

    def test_repeated_units_from_one_event_do_not_create_independent_evidence(self) -> None:
        events = []
        for index in range(econ.MIN_EVENT_CLUSTERS_FOR_EVIDENCE):
            oid = f"o-{index}"
            events.extend([
                order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=oid, leg_id="YES", event_id="same-event"),
                fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=oid, fill_id=f"f-{index}", leg_id="YES", event_id="same-event"),
                final(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=oid, pnl=1.0, event_id="same-event"),
            ])
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
            report = econ.assess(path, expected_model_sha=SHA)
        self.assertEqual(report["mature_terminal_units"], econ.MIN_EVENT_CLUSTERS_FOR_EVIDENCE)
        self.assertEqual(report["distinct_event_clusters"], 1)
        self.assertFalse(report["economic_evidence_ready"])
        self.assertIn("insufficient_distinct_event_clusters", report["reason_codes"])

    def test_twelve_distinct_positive_event_clusters_can_support_economic_evidence(self) -> None:
        events = []
        for index in range(econ.MIN_EVENT_CLUSTERS_FOR_EVIDENCE):
            oid = f"o-{index}"
            event_id = f"event-{index}"
            events.extend([
                order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=oid, leg_id="YES", event_id=event_id),
                fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=oid, fill_id=f"f-{index}", leg_id="YES", fee=0.0, event_id=event_id),
                final(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id=oid, pnl=1.0, slippage=0.0, unwind=0.0, capital=0.0, latency=0.0, event_id=event_id),
            ])
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "execution.jsonl"
            write_events(path, events)
            report = econ.assess(path, expected_model_sha=SHA)
        self.assertEqual(report["distinct_event_clusters"], econ.MIN_EVENT_CLUSTERS_FOR_EVIDENCE)
        ...
        self.assertTrue(report["economic_evidence_ready"])

    def test_missing_final_event_identity_fails_closed_for_evidence(self) -> None:
        events = [
            order(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", leg_id="YES", family="crypto_informed_taker", horizon=30, event_id="event-a"),
            fill(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", fill_id="f1", leg_id="YES", family="crypto_informed_taker", horizon=30, event_id="event-a"),
            final(strategy="CRYPTO_SETTLEMENT_ENGINE", order_id="o1", family="crypto_informed_taker", horizon=30, pnl=1.0, event_id="event-a"),
        ]
