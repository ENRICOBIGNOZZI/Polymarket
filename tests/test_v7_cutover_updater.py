...
class V7CutoverUpdaterTest(unittest.TestCase):
    ...
    def test_sha_cutover_drains_durable_paper_positions_before_stop_and_archive(self) -> None:
        ...
        self.assertIn("'source') == 'shadow_markout_and_fillability_observers'", function)
        self.assertIn("abs(int(runtime.get('timestamp',0))-int(maker_status.get('timestamp',-31))) <= 30", function)
        self.assertIn("isinstance(maker_status.get('observer_pids'),dict)", function)
        self.assertIn("int(maker_status.get('open_orders',-1)) == 0", function)
        self.assertIn("int(maker_status.get('open_positions',-1)) == 0", function)
        self.assertIn("maker_state={'inventory':{}}", function)
        self.assertIn("Maker only needs to prove entry is frozen", function)
        self.assertIn('runtime_pid="$(production_pid)"', function)
        self.assertIn("runtime_alive=sys.argv[3] == '1'", function)
        self.assertIn("if runtime_alive:", function)
        ...
        self.assertIn("abs(equity-budget) <= 1e-9", function)
        self.assertNotIn("maker_unmarkable", function)
        ...
        self.assertLess(refresh, finalizer)
        self.assertLess(finalizer, stop_monitoring)
    ...
    def test_launchd_monitoring_startup_has_bounded_convergence_window(self) -> None:
        ...
        self.assertIn("POLYMARKET_MONITORING_STOP_GRAFANA_LISTENER=0", start)
        self.assertIn("POLYMARKET_MONITORING_CONFIGURE_TAILNET=0", start)
        self.assertIn('monitoring_state="$APP_DIR/runs/monitoring/config"', start)
        self.assertIn('POLYMARKET_STATE_DIR="$monitoring_state"', start)
        self.assertIn('"$run_root" "$monitoring_state"', start)
        self.assertIn("for _ in $(seq 1 600); do", start)
