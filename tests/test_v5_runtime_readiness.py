...
class V5RuntimeReadinessTest(unittest.TestCase):
    ...
    def test_long_first_tick_is_ready_during_bounded_startup_grace(self) -> None:
        self._write_runtime(status_age=5000.0, started_at=self.NOW - 90.0)
        result = evaluate(self.root, now=self.NOW)
    ...
    def test_stale_output_after_startup_grace_fails_closed(self) -> None:
        self._write_runtime(status_age=5000.0, started_at=self.NOW - 601.0)
        with self.assertRaises(ReadinessError):
            evaluate(self.root, now=self.NOW)

    def test_dead_model_fails_even_during_startup_grace(self) -> None:
        self._write_runtime(status_age=5000.0, started_at=self.NOW - 30.0, model_alive="0")
        with self.assertRaises(ReadinessError):
            evaluate(self.root, now=self.NOW)
