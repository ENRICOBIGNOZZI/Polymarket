...
SPEC = importlib.util.spec_from_file_location(
    "attach_external_evidence", ROOT / "scripts" / "attach_external_evidence.py"
)
    ...
    def setUp(self) -> None:
        ...
        self.external = {
            ...
            "alpha_factory_evidence": {
                ...
                "specification": "kalshi:external_probability:21600s",
                ...
                "reasons": [],
                "critical_failures": [],
                "metrics": {"incremental_utility": 0.04},
            },
        }

    def test_attaches_research_candidate_without_self_promotion(self) -> None:
        report, state = adapter.attach(
            self.alpha, self.state, self.external, now=self.now, max_age_seconds=10800
        )
        self.assertTrue(report["diagnostics"]["external"]["fresh"])
        ...
        self.assertEqual(
            report["next_experiments"][0]["experiment_id"],
            "external_exact_clob_replay_and_incumbent_ablation",
        )

    def test_stale_external_report_is_diagnostic_only(self) -> None:
        stale = json.loads(json.dumps(self.external))
        stale["generated_ts"] = self.now - 20000
        report, state = adapter.attach(
            self.alpha, self.state, stale, now=self.now, max_age_seconds=10800
        )
        self.assertFalse(report["diagnostics"]["external"]["fresh"])
        self.assertEqual(report["candidates"], [])
        self.assertNotIn("external_evidence_candidate", state)

    def test_rejects_external_report_that_claims_execution(self) -> None:
