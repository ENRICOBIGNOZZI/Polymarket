...
class V7RuntimeChaosContractTest(unittest.TestCase):
    ...
    def test_restart_rebuilds_safe_paper_inventory(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._base(root)
            self._write(root / "external_fair/paper_router_status.json", {
                "paper_only": True, "authenticated_execution": False,
                "real_order_submission": False, "open_positions": 1,
                "pending_maker_orders": 0,
            })
            result = assess_reconciliation(root, SHA, now=1001)
            self.assertEqual(result.classification, RECOVERABLE)
            self.assertTrue(result.paper_inventory_present)
            self.assertIn("paper_inventory_reconciliation_required", result.reasons)
    ...
    def test_failure_isolation_matrix_covers_deterministic_chaos_scenarios(self) -> None:
        ...
        expected = {
            ...
            "pure_arb_clob_reject": "unwind_filled_legs_and_quarantine_market",
            "pure_arb_asymmetric_book_stale": "reject_opportunity_and_resnapshot",
            "market_fee_change": "cancel_reset_and_rebuild",
            "market_rollover_with_open_order": "hold_withdrawn_until_reconciled",
            "market_book_reset": "invalidate_book_and_resnapshot",
            "disk_io_stall": "withdraw_and_quarantine",
            "cpu_stall": "withdraw_and_quarantine",
        }
