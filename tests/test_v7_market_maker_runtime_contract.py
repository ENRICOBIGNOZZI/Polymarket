...
class ProfessionalMakerRuntimeContractTests(unittest.TestCase):
    ...
    def test_policy_requires_single_v7_architecture_and_bounded_exploration(self) -> None:
        ...
        self.assertEqual(architecture["slow_path"], "python_crypto_selection_frozen_model_inference")
    ...
    def test_canonical_runtime_keeps_maker_inside_native_owner(self) -> None:
        ...
        self.assertNotIn("v7_global_portfolio_coordinator.py", source)
        self.assertNotIn("polymarket_v7_authorized_maker_paper_executor", source)
        self.assertNotIn("v7_market_maker_rewards.py", source)
        self.assertNotIn("v7_maker_cohort_supervisor.py", source)
        self.assertIn("scripts/v7_native_crypto_engine_manager.py", source)
        self.assertIn("PM_V7_CRYPTO_SETTLEMENT_ENGINE", source)
        self.assertIn("build/polymarket_v7_crypto_settlement_engine", source)
        ...
        self.assertIn('MAKER_RESEARCH_MODEL="$RUN_ROOT/micro_maker/execution_model.json"', source)
        self.assertIn('export PM_V7_MAKER_EXECUTION_MODEL="$MAKER_RESEARCH_MODEL"', source)
        self.assertNotIn("v7_maker_durable_learning.py", source)
    ...
    def test_execution_cells_are_exact_sha_slow_path_and_hot_path_bounded(self) -> None:
        ...
        model = (ROOT / "scripts" / "v7_maker_durable_learning.py").read_text(encoding="utf-8")
        ...
        self.assertIn("censored_survival_hazard_joint_cycle_v4", model)
        self.assertIn("hazard_model", model)
        self.assertNotIn("filesystem", kernel)
        self.assertNotIn("boost/json", kernel)

    def test_london_runtime_is_inference_only_and_research_fit_is_external(self) -> None:
        loop = (ROOT / "scripts" / "paper_v7_execution_loop.sh").read_text(encoding="utf-8")
        research = (ROOT / "research" / "build_runtime_artifacts.sh").read_text(encoding="utf-8")
        self.assertNotIn("v7_maker_durable_learning.py", loop)
        self.assertNotIn("v7_external_rich_train.py", loop)
        self.assertNotIn('--output-model "$MAKER_RESEARCH_MODEL"', loop)
        self.assertIn("v7_runtime_artifacts.py", loop)
        self.assertIn("v7_maker_durable_learning.py", research)
        self.assertIn("v7_external_rich_train.py", research)
        self.assertNotIn("MAKER_MODEL_REGISTRY", loop)
    ...
    def test_markout_observer_is_evidence_only_and_full_depth(self) -> None:
        ...
        self.assertIn('"research" / "evidence" / "maker_markout"', observer)
        self.assertIn('metadata["research_evidence_only"] = true', observer)
        self.assertIn('metadata["ledger_writer_authority"] = false', observer)
        self.assertNotIn('"ledger" / "spool"', observer)
        self.assertIn("kHorizonMilliseconds{{", observer)
        self.assertIn("100, 250, 500, 1'000, 5'000, 10'000, 30'000, 45'000, 60'000, 300'000", observer)
        self.assertIn('"100ms", "250ms", "500ms", "1s", "5s", "10s", "30s", "45s", "60s", "300s"', observer)
        self.assertIn('metadata["horizon_ms"]', observer)
        self.assertIn('metadata["measurement_delay_ms"]', observer)
