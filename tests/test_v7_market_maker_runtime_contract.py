...
class ProfessionalMakerRuntimeContractTests(unittest.TestCase):
    ...
    def test_canonical_runtime_starts_maker_observers_without_independent_authority(self) -> None:
        ...
        self.assertIn("v7_maker_durable_learning.py", source)
        ...
        self.assertIn('MAKER_RESEARCH_STORE="$DURABLE_ROOT/micro_maker/research_evidence.jsonl"', source)
        self.assertIn('export PM_V7_MAKER_EXECUTION_MODEL="$MAKER_RESEARCH_MODEL"', source)
        self.assertIn('--output-model "$MAKER_RESEARCH_MODEL"', source)
        self.assertIn('--model "$MAKER_RESEARCH_MODEL"', source)
        self.assertNotIn(': > "$MAKER_RESEARCH_STORE"', source)
    ...
    def test_research_runtime_has_no_maker_governance_or_legacy_evidence(self) -> None:
        loop = (ROOT / "scripts" / "paper_v7_execution_loop.sh").read_text(encoding="utf-8")
        self.assertIn("v7_maker_durable_learning.py", loop)
        self.assertIn('--source-root "$RUN_ROOT"', loop)
        self.assertIn('--output-model "$MAKER_RESEARCH_MODEL"', loop)
        self.assertNotIn("MAKER_MODEL_REGISTRY", loop)
        self.assertNotIn('--source-root runs/paper_v7_archives', loop)
