...
class ProfessionalMakerRuntimeContractTests(unittest.TestCase):
    ...
    def test_canonical_runtime_starts_maker_observers_without_independent_authority(self) -> None:
        ...
        self.assertIn('--execution-model "$MAKER_RESEARCH_MODEL"', source)
        ...
        self.assertIn('RUNTIME_ARTIFACT_ROOT=', source)
        ...
        self.assertIn('--model "$MAKER_RESEARCH_MODEL"', source)
        self.assertNotIn('--output-model "$MAKER_RESEARCH_MODEL"', source)
        self.assertIn('runtime_training":false', source)
        self.assertNotIn('--source-root runs/paper_v7_archives', source)
