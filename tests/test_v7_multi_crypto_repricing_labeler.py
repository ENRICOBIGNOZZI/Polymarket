...
def rehash(value):
    value['record_hash']=canonical_hash({k:v for k,v in value.items() if k!='record_hash'})
    return value


def test_labeler_cannot_mix_policies_or_feature_schemas_at_same_sha():
    for key in ('policy_hash','feature_schema_hash'):
        target=row(100,.6); target[key]='d'*64; rehash(target)
        try: label_rows([row(0,.5),target],policy())
        except ValueError as exc: assert 'MIXED_EXPERIMENT_LINEAGE' in str(exc)
        else: raise AssertionError('incompatible feature cohort mixed')


def test_labeler_cannot_cross_token_mapping_changes():
    target=row(100,.6); target['yes_token']='other-token'; rehash(target)
    try: label_rows([row(0,.5),target],policy())
    except ValueError as exc: assert 'CONTRACT_IDENTITY_CHANGED' in str(exc)
    else: raise AssertionError('different token semantics combined')


def test_recording_clock_cannot_precede_decision():
    target=row(100,.6); target['recorded_wall_ns']=target['decision_wall_ns']-1; rehash(target)
    try: validate_tape_record(target)
    except ValueError as exc: assert 'availability' in str(exc)
    else: raise AssertionError('backdated recording accepted')


def test_ambiguous_identical_timestamp_is_not_arbitrarily_ordered():
    try: label_rows([row(0,.5),row(100,.6),row(100,.7)],policy())
    except ValueError as exc: assert 'AMBIGUOUS_SIMULTANEOUS_FEATURE_CUT' in str(exc)
    else: raise AssertionError('ambiguous target selected')


def test_invalid_mid_is_censored_not_labeled():
    out,_=label_rows([row(0,.5),row(100,1.2)],policy())
    assert out[0]['labels']['100']['status']=='TARGET_BOOK_INVALID'
    assert out[0]['labels']['100']['delta_pm_yes'] is None
