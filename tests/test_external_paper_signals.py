...
class ExternalPaperSignalTests(unittest.TestCase):
    ...
    def test_stale_future_invalid_and_weak_mapping_rows_fail_closed(self) -> None:
        rows = [
            ...
            self.row(market_id="weakmap", mapping_score=0.69),
        ]
        self.assertEqual(bridge.materialize(rows, now=self.now, max_age_seconds=3600), [])

    def test_latest_valid_row_wins_per_market(self) -> None:
        rows = [
    ...
    def test_cli_writes_header_when_no_direct_probability_is_available(self) -> None:
