...
class ServerHealthDiagnosticsContractTest(unittest.TestCase):
    def test_exporter_health_failure_is_captured_before_fail_closed_exit(self):
        ...
        self.assertIn("private exporter health check failed after diagnostics", health)
        ...
        final_fail_pos = health.index("private exporter health check failed after diagnostics")
    ...
    def test_future_runtime_diagnostics_follow_contract_selected_state_root(self):
        health = (ROOT / ".github" / "workflows" / "server-health.yml").read_text(encoding="utf-8")
