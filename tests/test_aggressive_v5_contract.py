...
class AggressiveV5ContractTest(unittest.TestCase):
    def test_config_is_broad_aggressive_and_risk_bounded(self) -> None:
        ...
        self.assertEqual({row["name"] for row in enabled}, {"micro", "pca", "graph", "semantic", "external"})
        ...
        self.assertLessEqual(max(row["overrides"]["min_net_edge"] for row in enabled), 0.001)
