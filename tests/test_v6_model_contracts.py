...
class V6ModelContracts(unittest.TestCase):
    ...
    def test_capital_sleeves_are_exhaustive(self):
        ...
        total = sum(float(v6[k]) for k in (
            ...
            "multileg_capital_fraction", "external_capital_fraction", "reserve_fraction",
        ))
    ...
    def test_semantic_is_discovery_not_fair_value(self):
        ...
        self.assertIn("live fair-value mixture is non-promotable", arch["scheduler_directive"]["all_schedulers"])
        cfg = json.loads((ROOT / "config/paper_v6.json").read_text())
        self.assertEqual(float(cfg["semantic_shrink"]), 0.0)
        self.assertEqual(float(cfg["expert_weights"]["semantic"]), 0.0)

    def test_runtime_routes_relations_through_guard(self):
        loop = (ROOT / "scripts/paper_v6_loop.sh").read_text()
        self.assertIn("v6_intent_guard.py", loop)
        self.assertIn("relation_intents_raw.csv", loop)
        self.assertIn("polymarket_negrisk_arb", loop)
    ...
    def test_maker_graph_hard_is_demoted_and_stressed(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            td = Path(td)
            src, dst, status = td / "in.csv", td / "out.csv", td / "status.json"
            with src.open("w", newline="", encoding="utf-8") as h:
                w = csv.DictWriter(h, fieldnames=FIELDS); w.writeheader()
                w.writerow({
                    "bundle_id":"g1", "strategy":"GRAPH_HARD", "event_id":"e", "created_ts":now,
                    "mode":"MAKER", "expected_edge":0.01, "max_notional":10, "market_id":"m1",
                    "side":"YES", "weight":1, "limit_price":0.4,
                    "execution_deadline_ts":now+120, "hold_deadline_ts":now+3600,
                })
                w.writerow({
                    "bundle_id":"weak", "strategy":"STRUCTURAL", "event_id":"e2", "created_ts":now,
                    "mode":"MAKER", "expected_edge":0.00025, "max_notional":10, "market_id":"m2",
                    "side":"NO", "weight":1, "limit_price":0.4,
                    "execution_deadline_ts":now+120, "hold_deadline_ts":now+3600,
                })
            subprocess.run([
                sys.executable, str(ROOT / "scripts/v6_intent_guard.py"), "--input", str(src),
                "--output", str(dst), "--status", str(status), "--min-edge", "0.0002",
                "--stress-bps", "10", "--max-age-seconds", "240",
            ], check=True, capture_output=True, text=True)
            rows = list(csv.DictReader(dst.open(newline="", encoding="utf-8")))
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0]["strategy"], "GRAPH_RV")
            report = json.loads(status.read_text())
            self.assertEqual(report["relabeled_graph_hard_to_rv"], 1)
            self.assertEqual(report["rejections"]["stress_edge"], 1)
