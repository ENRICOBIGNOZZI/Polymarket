...
class V7PublicDnsRuntimeContractTests(unittest.TestCase):
    ...
    def test_paper_loop_routes_public_rest_and_resolves_ws_without_os_dns(self) -> None:
        ...
        self.assertEqual(source.count('while [[ ! -e "$KILL" ]] && { ! maker_selection_ready || ! fee_registry_ready; }; do'), 1)
        self.assertIn("v7_maker_cohort_supervisor.py", source)
        self.assertNotIn("--observer-only", source)
    ...
    def test_service_entrypoint_has_no_external_credentials_surface(self) -> None:
        entrypoint = (ROOT / "ops" / "v7_service_entrypoint.sh").read_text(encoding="utf-8")
        self.assertNotIn("v7_credentials.env", entrypoint)
        self.assertNotIn("SPORTRADAR", entrypoint)
        self.assertNotIn("KALSHI", entrypoint)
        self.assertNotIn("LIMITLESS", entrypoint)
