...
SPEC = importlib.util.spec_from_file_location(
    "promotion_gate", ROOT / "scripts" / "promotion_gate.py"
)
...
class PromotionGateTests(unittest.TestCase):
    def fixture(self):
        ...
        candidate = {
            "number": 20,
            "headRefName": "integration/alpha-x",
            "headRefOid": "b" * 40,
            "isDraft": False,
            "mergeStateStatus": "CLEAN",
            "labels": [],
            "statusCheckRollup": checks(MODULE.REQUIRED_CANDIDATE_CHECKS),
            "body": body,
        }
        source = {
            "number": 10,
            "headRefName": "research/alpha-x",
            "headRefOid": source_sha,
            "statusCheckRollup": checks(MODULE.REQUIRED_SOURCE_CHECKS),
            "body": body,
            "comments": [],
            "reviews": [],
        }
        evidence = {
            ...
            "metrics": {
                "oos_trades": 40,
                "oos_net_pnl_usd": 5.0,
                "stressed_1_5x_net_pnl_usd": 3.0,
                "stressed_2_0x_net_pnl_usd": 1.0,
                "max_drawdown": 0.05,
                "profit_factor": 1.30,
                "bootstrap_one_sided_pvalue": 0.04,
                "fdr_adjusted_pvalue": 0.05,
                "active_folds": 3,
                "positive_fold_fraction": 2.0 / 3.0,
                "incremental_utility": 0.20,
    ...
    def test_economic_candidate_passes_only_with_full_evidence(self) -> None:
    ...
    def test_economic_candidate_without_evidence_is_blocked(self) -> None:
        ...
        self.assertIn(
            "economic_promotion_requires_machine_readable_evidence", result["errors"]
        )

    def test_latest_negative_research_verdict_blocks_green_candidate(self) -> None:
        candidate, source, evidence, now = self.fixture()
        source["comments"] = [
            {
        ...
        self.assertFalse(result["eligible"])
        self.assertIn(
            "latest_research_verdict_blocks_promotion:MORE_EVIDENCE_REQUIRED",
            result["errors"],
        )

    def test_non_overlapping_independent_evidence_is_required(self) -> None:
        candidate, source, evidence, now = self.fixture()
    ...
    def test_operational_candidate_can_advance_without_alpha_evidence(self) -> None:
        candidate, source, _, now = self.fixture()
    ...
    def test_merge_requires_controller_authorization_label(self) -> None:
        candidate, source, evidence, now = self.fixture()
    ...
    def test_candidate_code_provenance_excludes_only_live_selector(self) -> None:
