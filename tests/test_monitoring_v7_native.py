...
class V7NativeMonitoringTest(unittest.TestCase):
    ...
    def test_degraded_native_context_is_operational_but_not_fully_ready(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "paper_v7_live"
            self._fixture(root)
            sha = self._sha()
            ...
            self._write(root / "control/native_engine_manager_status.json", {
                ...
                "launch_blocked_contexts": {blocked: {"reason": "VenueMetadataUnavailable"}},
