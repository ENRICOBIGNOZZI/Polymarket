...
def test_config_and_horizon_separation() -> None:
    ...
    assert len(scopes) == 3
    five = freeze(
        value, code_sha=SHA, horizon_seconds=300, registry=registry(),
    ...
    research = freeze(
        value, code_sha=SHA, horizon_seconds=14_400, registry=registry(),
        live_scope=live_scope(),
        latency_profile=latency(), maker_evidence=maker(),
    )
...
def test_missing_execution_truth_fails_closed_but_preserves_cancel_path() -> None:
    snapshot = freeze(
        config(), code_sha=SHA, horizon_seconds=300, registry=registry(),
        live_scope=live_scope(),
    )
...
def test_maker_and_taker_evidence_gates_are_independent() -> None:
    snapshot = freeze(
        config(), code_sha=SHA, horizon_seconds=900, registry=registry(),
        live_scope=live_scope(),
