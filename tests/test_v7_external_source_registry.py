...
class ExternalSourceRegistryTest(unittest.TestCase):
    def test_canonical_registry_is_v7_paper_only_and_fingerprinted(self) -> None:
        ...
        expected_crypto = {"BTC", "ETH", "SOL", "XRP", "DOGE", "BNB"}
        crypto_assets = {row["asset"] for row in value["sources"] if row["asset"] in expected_crypto}
        self.assertEqual(crypto_assets, expected_crypto)
        self.assertTrue(all(
            row["enabled"] is False and row["research_only"] is True
            for row in value["sources"] if row["asset"] in {"ETH", "SOL", "XRP", "DOGE", "BNB"}
        ))
        self.assertIn("binance_spot_dogeusdt", result["source_ids"])
        self.assertIn("bybit_spot_bnbusdt", result["source_ids"])
