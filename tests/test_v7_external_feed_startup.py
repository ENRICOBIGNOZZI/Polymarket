...
class V7ExternalFeedStartupTest(unittest.TestCase):
    def test_external_feed_is_materialized_before_external_engine_starts(self) -> None:
        ...
        self.assertIn("refresh_external(){", loop)
        self.assertIn('external_signals_file', loop)
    ...
    def test_periodic_refresh_uses_same_fail_closed_materializer(self) -> None:
        ...
        tail = loop[periodic: periodic + 180]
        self.assertIn("refresh_external", tail)
        self.assertNotIn("v7_external_bridge.py --output", tail)

    def test_candidate_local_gate_is_not_enough_for_paper_feed(self) -> None:
        report = {
