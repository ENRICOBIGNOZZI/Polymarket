...
class ModelGovernanceContractTest(unittest.TestCase):
    def test_live_runtime_uses_automatic_validated_promotion_policy(self):
        ...
        self.assertEqual(manifest["loop"], "scripts/paper_v5_safe_loop.sh")
        ...
        safe_loop = (ROOT / manifest["loop"]).read_text(encoding="utf-8")
        self.assertIn("multi_strategy_paper_safe.py", safe_loop)
        self.assertIn("v5_stale_watchdog.py", safe_loop)
        self.assertIn("model_operability_report.py", safe_loop)
        ...
        self.assertIn("loop=scripts/paper_v5_safe_loop.sh", completed.stdout)
