...
class ModelGovernanceContractTest(unittest.TestCase):
    ...
    def test_promotion_controller_decides_and_integration_scheduler_only_executes(self):
        ...
        self.assertIn("scripts/research_pr_policy.py", controller)
        self.assertIn("trusted_research_policy", controller)
        self.assertIn("source-comments-pages.json", controller)
        self.assertIn("source-reviews-pages.json", controller)
        self.assertIn('--source-research-json "$probe_dir/source-research.json"', controller)
    ...
    def test_post_merge_and_deployment_still_use_exact_validated_sha(self):
        ...
        self.assertIn("fetch-depth: 0", post_merge)
        self.assertNotIn("fetch-depth: 1", post_merge)
