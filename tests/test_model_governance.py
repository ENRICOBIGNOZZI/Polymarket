...
class ModelGovernanceContractTest(unittest.TestCase):
    ...
    def test_research_policy_keeps_research_isolated_and_allows_machine_label_on_integration(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            source_sha = "a" * 40
            source_branch = "research/new-alpha"
            integration_event = {"pull_request":{"head":{"ref":"integration/new-alpha"},"draft":False,"body":f"Source research PR/branch/commit: #123 / `{source_branch}` / `{source_sha}`\n","labels":[{"name":"autonomous-promotion-approved"}]}}
