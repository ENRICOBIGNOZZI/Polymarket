...
class HardSafetyPolicyTest(unittest.TestCase):
    ...
    def test_v7_rejects_100_percent_sandbox_and_other_overlimits(self) -> None:
        current = v7_envelope()
        current["max_market_fraction"] = 1.0
        current["max_event_fraction"] = 1.0
        current["max_gross_fraction"] = 1.0
        current["multi_strategy"]["global_max_gross_fraction"] = 1.0
        joined = "\n".join(compare_paper_config(BASE, current, "config/paper_v7.json"))
        self.assertIn("max_market_fraction allowed<=0.05, got 1", joined)
        self.assertIn("max_event_fraction allowed<=0.15, got 1", joined)
        self.assertIn("max_gross_fraction allowed<=0.7, got 1", joined)
        self.assertIn("global_max_gross_fraction allowed<=0.7, got 1", joined)

    def test_v7_kelly_trade_size_drawdown_and_paper_boundary_are_bounded(self) -> None:
        current = v7_envelope()
        current["fractional_kelly"] = 0.251
        current["max_trade_usd"] = 125.01
        current["max_drawdown"] = 0.151
