...
class V6RuntimeContractTest(unittest.TestCase):
    ...
    def test_v6_research_smoke_feeds_and_observes_tiny_exploration(self) -> None:
        ...
        self.assertIn("--improve-ticks 1 --max-queue-multiple 6 --once", workflow)
        self.assertIn("probe_ticks=72", workflow)
        self.assertIn('for i in $(seq 1 "$probe_ticks"); do', workflow)
        self.assertIn("if (( i < probe_ticks )); then sleep 5; fi", workflow)
