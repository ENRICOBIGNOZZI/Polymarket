...
def test_markout_is_valid_ledger_event(tmp_path: Path):
    ...
    assert out["total"]["markout_count"]["10s"] == 1


def test_prometheus_has_segment_and_conditional_metrics():
    empty = {
    ...
    assert 'polymarket_maker_lab_segment_orders{action="JOIN",variant="JOIN",dimension="toxicity",bucket="LOW"} 5' in text
    assert 'polymarket_maker_lab_conditional_orders{action="JOIN",toxicity="LOW",queue="<=1x"} 5' in text
    assert 'polymarket_maker_lab_market_realized_pnl_usd{market="M1",action="JOIN"} 0.2' in text
