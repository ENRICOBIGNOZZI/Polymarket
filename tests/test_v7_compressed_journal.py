...
class JournalTests(unittest.TestCase):
    def test_locator_cache_has_no_payload_copies_or_disk_files_and_enforces_page_budget(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertFalse(hasattr(index,'db'))
            self.assertEqual(index.metrics['storage_backend'],'BOUNDED_PYTHON_LOCATOR_MAP')
            self.assertTrue(all(len(row[0])==32 and len(row[8])==32 for row in index.records.values()))
            self.assertLessEqual(index.metrics['index_bytes'],65536)
            self.assertEqual(index.metrics['database_bytes'],0)
