...
class IncumbentHealthGateTest(unittest.TestCase):
    ...
    def test_integration_workflow_promotes_without_incumbent_health_blocker(self):
        ...
        self.assertIn(
            "Scan numbered research sources and select first fully-green candidate",
            workflow,
        )
        self.assertIn("scripts/integration_gate.py validate", workflow)
        self.assertIn("statusCheckRollup", workflow)
        self.assertIn('--match-head-commit "$expected_head"', workflow)
        self.assertIn('"event_type": "champion-integration-merged"', workflow)
