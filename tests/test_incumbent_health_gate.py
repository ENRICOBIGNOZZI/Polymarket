...
class IncumbentHealthGateTest(unittest.TestCase):
    def run_gate(self, *, main_sha: str = SHA, validated_sha: str = SHA,
                 deploy_enabled: bool = True, health_text: str | None = None,
                 max_age_seconds: int = 7200) -> subprocess.CompletedProcess[str]:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            output = temp / "report.md"
            command = [
                "python3", str(SCRIPT), "--main-sha", main_sha,
                "--validated-sha", validated_sha,
                "--deploy-enabled", "true" if deploy_enabled else "false",
    ...
    @staticmethod
    def healthy_text(sha: str = SHA, recorder: str = "1", broker: str = "1",
                     timestamp: str = CURRENT_TIMESTAMP) -> str:
        return (
    ...
    def test_integration_workflow_consumes_controller_authorization(self):
        ...
        self.assertNotIn("Build locally-green automatic promotion queue", workflow)
        self.assertNotIn("scripts/incumbent_health_gate.py", workflow)

    def test_private_health_scheduler_remains_independent(self):
