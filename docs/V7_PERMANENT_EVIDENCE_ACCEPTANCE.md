...
The user superseded the earlier 30 GB constraint and authorized the managed `runs` data area up to **60,000,000,000 decimal bytes**. Current policy preserves raw detail until the 54 GB retention trigger when possible, then targets 50 GB; historical 30 GB measurements below remain audit history rather than the active limit.
...
- Counterfactual history now uses the bounded journal in the production router entrypoint, including startup recovery writes. Both writers are acquired before initialization. Segment-aware restoration, training and audit readers retain old generations without an active tail file; root-based deduplication prevents durable segments being copied back into themselves. The separate retention task relinquishes this producer-owned path.
- Eleven journal tests cover A→B model replacement, interrupted dual publication, compressed-only discovery, conflicts, incomplete sealed rows and compression publication races. The full 143-test suites ran in Release, Debug and ASan/UBSan. They exposed a package-import failure and a gzip hardlink-cleanup race; both were corrected and their failed tests passed on rerun. No native code changed. Full-history cache rebuild latency on rotation, the unbounded disposable SQLite index, global 30 GB enforcement and deployment remain open; this revision is not a storage-capacity acceptance claim.
- The temporary counterfactual payload cache has been replaced by bounded in-memory locators and raw/canonical hashes. The database page ceiling is 256 MiB per index, with no disk cache. Queries verify each returned raw record; quota exhaustion is explicit and preserves the sources. Exact prefix verification allows ordinary rotation/compression to reuse old index entries rather than reparsing all historical JSON. Fifteen journal/index tests cover quotas, byte mutation, paused query snapshots, rotation/rank preservation and the prior restart cases.
- Full current durable-prefix audit on Python 3.14.7: **1,238,028,545 source bytes; 180,771 records; 67,919,872 database bytes; zero disk-cache bytes**. Initial indexing took 87.50 seconds; full verified iteration took 22.48 seconds; unchanged refresh took 0.000324 seconds. This is a current-source benchmark, not end-to-end router latency or a whole-system memory bound. Artifact `counterfactual-locator-index-final-audit.json` records the prefix SHA-256 and implementation SHA-256; the exact benchmark implementation is preserved under `analysis_sources`. Startup scanning cost remains relevant to safe restart admission.
- After the locator/rotation changes, all **143 CTest entries passed in Release, Debug and ASan/UBSan** (macOS leak detection disabled). The temporary-index disk-growth issue is addressed in the proposed code; the all-data 30 GB ceiling, sustainable long-term raw retention, full-history materialization and live deployment remain incomplete.
- Exact-commit Linux CI for `32f6eda43cb514d1c12b13086367bbea23b81da8` exposed a rotation-performance regression: all three builds reparsed 1,004 records instead of only the two new records. Verified newly sealed prefixes now take precedence over inode equality, covering immediate inode reuse after compression. The regression test reproduces inode reuse on macOS as well; all 15 journal/index tests and the standalone router suite pass locally. Deployment has not begun; exact-SHA CI must pass for the correction first.
