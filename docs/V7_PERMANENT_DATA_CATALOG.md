...
The same bounded journal now preserves coordinator result publications, pinned selector publications and the three public REST collector tapes. Complete records remain available in ordered `.segment-*.jsonl.gz` files with decoded-hash receipts. A writer lock prevents competing producers. Operator logs retain errors while structured publication history moves to the declared event-journal paths. Existing legacy files remain preserved.

Confirmatory cohorts additionally retain `confirmatory_window_closure.json`, `confirmatory_final.json` and `final_analysis_sources/*.py.gz`. The producer closes the observed window after pending observations drain; the report verifies that closure, terminal records and settlement provenance before publishing one immutable final. Missing primary causal coverage prevents confirmatory inference, and later changes remain visible without replacing the frozen result.

The router entrypoint now acquires both counterfactual journal writers before startup reconciliation. Active and durable forecast histories rotate into verified 64 MiB gzip segments. Startup recovery, rich-model training, forward audits and cross-run economic audits discover the segments even when the hot pathname is absent. Durable reconciliation uses the logical source root to avoid copying its own older segments again. The external retention task no longer rotates the router-owned counterfactual path.

The counterfactual index stores only row locators and hashes in an in-memory SQLite database, capped at 256 MiB of database pages per index; at most two indexes are cached. It creates no disk cache or disk sort files. This is a database-page bound, not a process-RSS bound. Every returned row is read from its source and verified against its exact raw SHA-256; a separate canonical-JSON hash detects conflicting duplicate IDs across differently formatted copies. Mutable descriptors preserve the query snapshot during rotation, and at most eight sealed source streams remain open per iterator. Verified prefix moves reuse prior index entries; an unverified replacement, truncation or rewrite still rebuilds. Exhausting the index budget fails closed with `CACHE_BUDGET_EXHAUSTED_SOURCES_PRESERVED`, without deleting source evidence or returning a partial population as complete. Accumulated compressed history still requires global capacity enforcement. These changes have not been deployed.
