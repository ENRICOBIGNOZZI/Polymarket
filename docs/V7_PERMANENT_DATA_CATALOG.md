...
The router entrypoint now acquires both counterfactual journal writers before startup reconciliation. Active and durable forecast histories rotate into verified 64 MiB gzip segments. Startup recovery, rich-model training, forward audits and cross-run economic audits discover the segments even when the hot pathname is absent. Durable reconciliation uses the logical source root to avoid copying its own older segments again. The external retention task no longer rotates the router-owned counterfactual path. The disposable counterfactual index rebuilds when partition topology changes; unchanged refreshes and ordinary appends remain incremental. This adds bounded hot files, but does not bound the accumulated compressed history or the disposable SQLite index, and has not yet been deployed.
