...
sudo install -d -o "$SERVICE_USER" -g "$SERVICE_GROUP" "$(dirname "$APP_DIR")" "$RUN_ROOT" "$RUNTIME_ROOT" "$RUNTIME_ROOT/by-sha" "$RUNTIME_ROOT/ci-receipts" "$ARTIFACT_ROOT/by-sha" "$(dirname "$RUN_ROOT")/paper_v7_london_archives"
