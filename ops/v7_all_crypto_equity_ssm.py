#!/usr/bin/env python3
"""Run all-crypto timing-equity research on London PAPER evidence."""
...
def main():
    ...
    remote="/tmp/polymarket-all-crypto-equity-"+a.expected_sha[:12]
...
PYTHONPATH={remote}/src:{ctx['app_dir']} nice -n 18 venv/bin/python -m research.walk_forward_v3.all_crypto_compact_equity \
  --root {ctx['run_root']} \
  --minimum-wall-ns {a.minimum_wall_ns} \
  --output {remote}/output/all_crypto_compact_equity.json
python3 - {remote}/output/all_crypto_compact_equity.json <<'PY'
import json,sys
v=json.load(open(sys.argv[1]))
assert v['schema']=='polymarket_v7_all_crypto_compact_timing_equity_v1'
...
assert v['latencies_ms']==[10,25,50,100,250]
assert v['exit_horizons_ms']==[500,750,1000,1500,2000,3000,4000,5000]
print('ALL_CRYPTO_EQUITY_READY')
print(json.dumps({{'assets':v['assets'],'contract_horizons':v['contract_horizons'],
                   'tape_diagnostics':v['tape_diagnostics']}},sort_keys=True))
PY
...
print('ALL_CRYPTO_RESULT='+json.dumps({{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}},sort_keys=True))
PY"""
    stdout,_=run(REGION,a.instance_id,cmd,5400)
    marker=next(line for line in stdout.splitlines() if line.startswith("ALL_CRYPTO_RESULT="))
    ...
    print("all_crypto_equity_output="+str(a.output_dir))
    return 0
