#!/usr/bin/env python3
"""Run the no-ML simple-edge diagnostic on London PAPER evidence."""
from __future__ import annotations
import argparse,json,re,tarfile,io
...
PATHS=(
    ...
    "research/walk_forward_v3/simple_edge_diagnostic.py",
...
def main():
    ...
    remote="/tmp/polymarket-simple-edge-"+a.expected_sha[:12]
...
PYTHONPATH={remote}/src:{ctx['app_dir']} nice -n 19 venv/bin/python -m research.walk_forward_v3.simple_edge_diagnostic \
  --root {ctx['run_root']} \
  --minimum-wall-ns {a.minimum_wall_ns} \
  --output {remote}/output/simple_edge.json
python3 - {remote}/output/simple_edge.json <<'PY'
import json,sys
v=json.load(open(sys.argv[1]))
assert v['schema']=='polymarket_v7_simple_edge_diagnostic_v1'
...
print('SIMPLE_EDGE_RESULT='+json.dumps({{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}},sort_keys=True))
PY"""
    stdout,_=run(REGION,a.instance_id,cmd,1800)
    marker=next(line for line in stdout.splitlines() if line.startswith("SIMPLE_EDGE_RESULT="))
    ...
    print("simple_edge_output="+str(a.output_dir))
    return 0
