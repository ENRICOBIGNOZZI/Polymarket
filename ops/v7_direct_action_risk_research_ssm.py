...
SOURCE_PATHS = (
    ...
    "research/walk_forward_v3/run_risk_frontier.py",
...
def load_request(path: Path) -> dict:
    ...
    if set(value) != required:
        raise ValueError("unexpected risk request fields")
    if value["schema"] != "polymarket_v7_direct_action_risk_research_ssm_request_v1":
        raise ValueError("invalid risk request schema")
    if value["version"] != 1:
        raise ValueError("invalid risk request version")
    ...
    if not re.fullmatch(
        r"docs/research/direct-action-risk-[0-9]{4}-[0-9]{2}-[0-9]{2}",
...
PYTHONPATH={remote}/src:{app_dir} venv/bin/python -m research.walk_forward_v3.run_risk_frontier \
  --input-root {run_root} \
  --output {remote}/output/risk_frontier.json \
...
print('DIRECT_ACTION_RISK_RESULT='+json.dumps(
...
PY"""
    ...
    line = next(
        ...
        if item.startswith("DIRECT_ACTION_RISK_RESULT=")
    )
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: v7_direct_action_risk_research_ssm.py REQUEST SOURCE_SHA")
    ...
    remote = "/tmp/polymarket-direct-action-risk-" + source_sha[:12]
    ...
    print("DIRECT_ACTION_RISK_LOCAL_OUTPUT=" + request["output_directory"])
    return 0
