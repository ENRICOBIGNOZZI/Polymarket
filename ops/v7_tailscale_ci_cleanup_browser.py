...
PREFIXES = ("gh-deploy-", "gh-health-", "gh-freeze-", "gh-inventory-", "github-runnervm", "github-actions")
GH_NAME_RE = re.compile(r"^(gh-(?:deploy|health|freeze|inventory)-([0-9]+)-([0-9]+))$", re.I)
GENERIC_CI_RE = re.compile(r"^(?:github-runnervm[a-z0-9-]+|github-actions[a-z0-9-]+)$", re.I)
INTERACTIVE_MARKERS = (
...
def login(page, email: str, password: str) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if any(marker.lower() in body.lower() for marker in INTERACTIVE_MARKERS):
            if not wait_for_human_2sv(page, timeout_s=420):
                raise RuntimeError("interactive_auth_timeout")
...
def candidate_names(page) -> list[str]:
    ...
    names: set[str] = set()
    for term in ("gh-", "github-runnervm", "github-actions"):
        if search is not None:
            search.fill(term)
            page.wait_for_timeout(1000)
        body = page.locator("body").inner_text(timeout=10000)
        names.update(m.group(1) for m in re.finditer(
            r"\b(gh-(?:deploy|health|freeze|inventory)-[0-9]+-[0-9]+)\b", body, re.I
        ))
        names.update(m.group(0) for m in re.finditer(
            r"\b(?:github-runnervm[a-z0-9-]+|github-actions[a-z0-9-]+)\b", body, re.I
        ))
    if search is not None:
        search.fill("")
...
def offline_evidence(text: str) -> bool:
    low = text.lower()
    if re.search(r"\boffline\b|\bexpired\b|last seen|last connected|\bdisconnected\b", low):
        return True
    return bool(re.search(r"\b\d+\s*(?:m|min|mins|minute|minutes|h|hr|hour|hours|d|day|days|w|week|weeks)\s+ago\b", low))

def remove_one(page, name: str) -> None:
    row = row_for_name(page, name)
    if not offline_evidence(row.inner_text(timeout=5000)):
        raise RuntimeError(f"no_offline_evidence:{name}")
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        for name in candidates:
            ...
            if match is not None and int(match.group(2)) in protected:
                continue
            if match is None and not GENERIC_CI_RE.fullmatch(name):
                continue
    ...
    print(json.dumps({
        ...
        "protected_count":sum(1 for n in candidates if (GH_NAME_RE.fullmatch(n) is not None and int(GH_NAME_RE.fullmatch(n).group(2)) in protected)),
