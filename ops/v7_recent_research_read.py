...
for p in root.rglob('*'):
 ...
 if ('native' in rel or 'probability' in rel or 'settlement' in rel or 'crypto' in rel) and p.suffix in ('.json','.jsonl','.gz'):
  ...
  if p.suffix=='.json' and 'status' in name and p.stat().st_size<100000:
   try:statuses.append(dict(path=rel,value=json.loads(p.read_text())))
   except ValueError:pass
  if '.jsonl' in name and len(samples)<12:
   try:
    with (gzip.open(p,'rt') if name.endswith('.gz') else p.open()) as f:
     for index,line in enumerate(f):
      if index>500:break
      v=json.loads(line)
      if len(samples)<12 and not any(s['path']==rel and s['value'].get('kind')==v.get('kind') for s in samples):samples.append(dict(path=rel,value=v))
   except (ValueError,OSError):pass
result=dict(root=str(root),start_ms=int(start*1000),end_ms=int(now*1000),runtime=status,
 files=files,samples=samples,statuses=statuses)
